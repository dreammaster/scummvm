...
#ifndef GLK_GLULX_GLULXE_H
#define GLK_GLULX_GLULXE_H
...
extern void fatal_error_handler(const char *str, const char *arg, int useval, glsi32 val) GLK_ATTRIBUTE_NORETURN;
extern void nonfatal_warning_handler(const char *str, const char *arg, int useval, glsi32 val);
...
template <typename T>
inline T *glulx_malloc(uint len) {
	return (T *)malloc(len);
}
template <typename T>
inline T *glulx_realloc(T *ptr, uint len) {
	return (T *)realloc(ptr, len);
}
...
extern void profile_fail(const char *reason);
extern void profile_quit(void);
#else /* VM_PROFILING */
inline void profile_tick() {}
inline void profile_profiling_active() {}
inline void profile_in(glui32 addr, glui32 stackuse, int accel) {}
inline void profile_out(glui32 stackuse) {}
inline void profile_fail(const char *reason) {}
inline void profile_quit() {}
#endif /* VM_PROFILING */
...
extern void debugger_block_and_debug(const char *msg);
extern void debugger_handle_crash(const char *msg);
extern void debugger_handle_quit(void);
#else /* VM_DEBUGGER */
inline void debugger_tick() {}
inline void debugger_check_story_file() {}
inline void debugger_setup_start_state() {}
inline void debugger_check_func_breakpoint(glui32 addr) {}
inline void debugger_handle_crash(const char *msg) {}
#endif /* VM_DEBUGGER */
