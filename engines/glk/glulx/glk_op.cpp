...
/* Glulxe code for Glk API dispatching.
 */
...
static gidispatch_rock_t glulxe_retained_register(void *array,
  glui32 len, const char *typecode);
static void glulxe_retained_unregister(void *array, glui32 len, 
  const char *typecode, gidispatch_rock_t objrock);
...
static void prepare_glk_args(const char *proto, dispatch_splot_t *splot);
static void parse_glk_args(dispatch_splot_t *splot, const char **proto, int depth,
  int *argnumptr, glui32 subaddress, int subpassin);
static void unparse_glk_args(dispatch_splot_t *splot, const char **proto, int depth,
...
*/
static const char *read_prefix(const char *cx, int *isref, int *isarray,
  ...
  for (argx = 0, ix = 0; argx < numwanted; argx++, ix++) {
    ...
    if (!skipval) {
      ...
      if (typeclass == '[') {
        ...
        case 'I':
          if (*cx == 'u')
            garglist[gargnum]._uint = (glui32)(thisval);
          else if (*cx == 's')
            garglist[gargnum]._sint = (glsi32)(thisval);
          else
            fatal_error("Illegal format string.");
          gargnum++;
          cx++;
    ...
    if (!skipval) {
      ...
      if (typeclass == '[') {
        ...
        case 'C':
          ...
          break;
        case 'I':
          ReleaseIArray((glui32 *)garglist[gargnum]._array, varglist[ix], varglist[ix+1], passout);
          gargnum++;
          ix++;
          gargnum++;
          cx++;
        ...
        case 'I':
          if (!skipval) {
            if (*cx == 'u')
              thisval = (glui32)garglist[gargnum]._uint;
            else if (*cx == 's')
              thisval = (glui32)garglist[gargnum]._sint;
            else
              fatal_error("Illegal format string.");
          }
          gargnum++;
        ...
        case 'C':
          if (!skipval) {
            if (*cx == 'u') 
              thisval = (glui32)garglist[gargnum]._uch;
            else if (*cx == 's')
              thisval = (glui32)garglist[gargnum]._sch;
            else if (*cx == 'n')
              thisval = (glui32)garglist[gargnum]._ch;
            else
              fatal_error("Illegal format string.");
          }
          gargnum++;
...
static gidispatch_rock_t glulxe_retained_register(void *array,
  ...
  if (!arref)
    fatal_error("Unable to re-find array argument in Glk call.");
  if (arref->elemsize != (uint)elemsize || arref->len != len)
    fatal_error("Mismatched array argument in Glk call.");
  ...
  const char *typecode, gidispatch_rock_t objrock)
{
  ...
  if (arref->elemsize != (glui32)elemsize || arref->len != len)
    fatal_error("Mismatched array argument in Glk call.");
