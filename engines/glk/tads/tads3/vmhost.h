...
#ifndef TADS3_VMHOST_H
#define TADS3_VMHOST_H

/* T3 VM host interface
 *
 * The host interface defines a set of services that the VM obtains from its host
 * application environment.  The VM uses the host interface to obtain these services
 * so that the VM doesn't have to make too many assumptions about the larger application
 * of which it is a subsystem. Every application containing the VM must provide the VM
