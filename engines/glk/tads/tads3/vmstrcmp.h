...
#ifndef TADS3_VMSTRCMP_H
#define TADS3_VMSTRCMP_H

/* T3 String Comparator intrinsic class
 *
 * Defines the String Comparator intrinsic class, which provides native code that performs complex,
 * parameterized string comparisons.  We offer the following customizable options for our comparisons:
 *
 * - We can match exactly on case, or without regard to case.
 *
 * - We can optionally match a value to a truncated reference value (which allows user input to use
 * abbreviated forms of dictionary words, for example).  The minimum truncation length is a settable option.
 *
 * - We can use equivalence mappings that allow a given character in a reference string to match different
 * characters in value strings.  For example, we could specify that an "a" with an acute accent in a reference
 * string matches an unaccented "a" in a value string.  Each such mapping can specify result flag bits, so a caller
 * can determine if particular equivalence mappings were used in making a match.
 *
 * This class implements the generic "comparator" interface, by providing a hash value calculator method and a value
 * comparison method, so a String Comparator instance can be used as a Dictionary's comparator object.
 *
 * StringComparator objects are immutable; all of our parameters are set in the constructor.  This is desirable because
 * it allows the object to be installed in a Dictionary (or any other hash table-based structure)
 * without any danger that the hash table will need to be rebuilt as long as the same comparator is installed.
 */
