...
/* UTF-8 character string manipulation */
...
public:
	 ...
	 */
	utf8_ptr() : _pc(nullptr), _pnc(nullptr) {}
	 ...
	 */
	wchar_t getch() const { return s_getch(_pc); }

	/**
	 * Get the character at a given character offset from the current position.
	 * The offset must be positive.  
	 */
	wchar_t getch_at(size_t ofs) const { return s_getch_at(_pc, ofs); }

	/**
	 * Get the character preceding the current character by the given amount.
	 * The offset must be positive.  getch_before(1) returns the character
	 * preceding the current character, (2) returns the character two positions
	 * before the current character, and so on. 
	 */
	wchar_t getch_before(size_t ofs) const { return s_getch_before(_pc, ofs); }
	 ...
	 * string pointer is left at the next character after the last copied character.
	 */
	...
	/**
	 * Increment by a byte count
	 */
	void inc_bytes(size_t cnt) {
		if (_pnc) {
			_pnc += cnt;
			_pc = _pnc;
		} else {
	...
	/**
	 * Decrement by a byte count
	 */
	void dec_bytes(size_t cnt) {
		if (_pnc) {
			_pnc -= cnt;
			_pc = _pnc;
		} else {
	 ...
	 */
	int is_continuation() const { return s_is_continuation(_pc); }

	/** 
	 * Count the number of characters in the given number of bytes,
	 * starting at the current byte 
	 */
	size_t len(size_t bytecnt) const { return s_len(_pc, bytecnt); }

	/**
	 * Get the byte size of the current character
	 */
	size_t charsize() const { return s_charsize(*_pc); }
    
	/**
	 * Get the number of bytes in the given number of characters
	 * starting at the current position.  
	 */
	size_t bytelen(size_t charcnt) const { return s_bytelen(_pc, charcnt); }
	...
	size_t lenz() const {
		...
		for (cnt = 0, p = _pc ; *p != 0 ; p = s_inc(p), ++cnt) ;
	...
	*/
	static int s_compare_to(const char *p1, size_t bytelen1, const char *p2, size_t bytelen2);

	/* get the character at the given byte pointer */
	static wchar_t s_getch(const char *p) {
		// If the high bit is 0, it's a one-byte sequence encoding the value in the low seven bits.  
		if ((*p & 0x80) == 0)
	...
	/** 
	 * Get the character at a given positive character offset from a byte pointer 
	 */
	...
	static wchar_t s_getch_before(const char *p, size_t ofs) {
		...
		for ( ; ofs != 0 ; --ofs) {
			//Back up by one to three bytes, until we find no more continuation flags 
			--p;
	 ...
	 * enough room in the buffer for this character's UTF-8 representation.
	 *   
	...
	static size_t s_putch(char *p, wchar_t ch) {
		...
		if (ch <= 0x7f) {
			...
			*p = (char)(ch & 0x7f);
			...
			/* 
			 * It's in the range 0x0080 to 0x07ff - encode it in two bytes.
			 * The high-order 5 bits go in the first byte after the two-byte
			 * prefix of 110, and the low-order 6 bits go in the second byte
			 * after the continuation prefix of 10.  
			 */
	 ...
	 */
	static const char *s_inc(const char *p) {
		// Increment the pointer by the size of the current character and return the result
	...
	/**
	 * Get the number of bytes required to encode a given wchar_t in UTF-8 format 
	 */
	...
	static char *s_dec(char *p) {
		 ...
		 * unroll the loop and simply check twice to see if we're done yet.  
		 */
		--p;
		p -= s_is_continuation(p);
		p -= s_is_continuation(p);
	 ...
	 * Decrement a pointer by one character, returning the result
	 */
	static const char *s_dec(const char *p) {
		--p;
		p -= s_is_continuation(p);
		p -= s_is_continuation(p);
	 ...
	 */
	static size_t to_wcharz(wchar_t *buf, size_t bufcnt, const char *str, size_t len) {
