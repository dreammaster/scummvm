...
#ifndef TADS3_VMTMPFIL_H
#define TADS3_VMTMPFIL_H

/* CVmObjTemporaryFile object
 *
 * The TemporaryFile intrinsic class describes a temp file on disk.  This isn't an open
 * instance of a temp file; rather, it basically represents a filename in the local file system.
 *
 * The reason to make this an object is that it provides a natural way of identifying and freeing
 * temp files.  The File class can validate that the program is supplying a valid temporary file
 * generated by the system, because only the system can create a TemporaryFile object.
 * This prevents a malicious program from trying to circumvent the file safety levels by faking
 * a temp file name; it's essentially impossible to invent a fake name since the name is encapsulated
 * in the system object.  The system can tell when it's safe to delete the underlying file system
 * object through the magic of garbage collection: when the TemporaryFile object is collected,
 * the program is definitely done with it, since the program doesn't have any references to it.
 */
