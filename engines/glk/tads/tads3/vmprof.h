...
#ifndef TADS3_VMPROF_H
#define TADS3_VMPROF_H

/* T3 VM Profiler
 *
 * Definitions for the execution profiler, which captures information on the time the program
 * spends in each function.  Profiling information is useful for performance optimization,
 * because it identifies the areas where the program is spending all of its time.
 *
 * Profiling can be optionally included when the VM is compiled.  Builds of the VM intended
 * for use by end users should normally not include the profiler, because they will have no
 * use for profiling, and including the profiler in the build enlarges the code size and adds
 * run-time overhead.  Normally, the profiler should be included only in builds that include
 * the interactive debugger.
 */

#include "glk/tads/tads3/vmtype.h"
#include "glk/tads/tads3/vmprofty.h"
