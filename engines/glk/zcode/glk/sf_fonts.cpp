...
static SF_bdffont *sBDXload(FILE *f, int *err, int *size, int MAXCHAR) {
	...
	byte *po, *pbeg;
	char *p, *q;
	char *fontname = (char *)"", *copyright = (char *)"unknown";
	int fngot = 0, cpgot = 0;
	int hasenc, hasbbx;
	SF_bdffont *font = nullptr;
...
static SFONT *makefont(SF_bdffont *b) {
	...
	if (!b)
		return nullptr;
	res = (SFONT *)calloc(1, sizeof(SFONT));
	if (!res) {
		free(b);
		return nullptr;
	}
...
void sf_initfonts() {
	...
	SFONT *Norm, *Emph = nullptr, *Bold = nullptr, *Bemp = nullptr;
...
char *sf_searchfile(char *fn, int fnlen, char *buf, const char *paths) {
	...
	for (;;) {
		int plen;
		p = (char *)strchr(paths, OS_PATHSEP);
		if (p)
			plen = p - paths;
		else
			plen = strlen(paths);
