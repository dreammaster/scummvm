...
static /*SDL_Renderer*/void *renderer = nullptr;
static /*SDL_Texture*/void *texture = nullptr;
...
int SFdticks = 200;
static /*SDL_atomic_t*/uint SFticked = 0;
...
#else
	error("TODO: sf_toggle_fullscreen");
#endif
}
...
void sf_initvideo(int W, int H, int full) {
	...
	timerid = SDL_AddTimer(SFdticks, mytimer, nullptr);
...
void os_draw_picture(int picture, int y, int x) {
	...
	if (pic->usespalette) {
		...
		for (yy = 0; yy < eh; yy++) {
			for (xx = 0; xx < ew; xx++) {
				dst =
				    sbuffer + x + (uint32)(xx * m_gfxScale_w) +
				    sbpitch * (y + (uint32)(yy * m_gfxScale_h));
				sval = src[xx];
				alpha = (sval >> 24);
				if (alpha == 255)
					dval = sval & 0xffffff;
...
static void handle_window_event(Common::Event *e) {
#ifdef TODO
...
#else
	error("TODO: handle_window_event");
#endif
}
...
ulong *sf_savearea(int x, int y, int w, int h) {
	...
	r = p = (ulong *)malloc((w * h + 4) * sizeof(ulong));
	if (!r)
		return nullptr;
