...
static void usage(int type) {
	...
	debug(info_header, VERSION);

	if (type == USAGE_NORMAL)
		p = info;
	else {
		p = extended_options;
		debugN(extended_header);
	}
...
void os_process_arguments(int argc, const char *argv[]) {
	...
	if (argv[zoptind] == nullptr) {
		usage(USAGE_NORMAL);
		os_quit(EXIT_SUCCESS);
	}
	f_setup.story_file = strdup(argv[zoptind]);

	if (argv[zoptind + 1] != nullptr)
...
char *os_read_file_name(const char *default_name, int flag) {
	...
	if (newfile(flag)) {
		const char *ext = getextension(flag);
		if (m_names_format == 'd')
...
char *sf_GetProfileString(const char *sect, const char *id, const char *def) {
	...
	char *result = def ? strdup(def) : nullptr;
	if (sav)
		*q = sav;
	return result;
}
...
static int myunzip(int csize, byte *cdata, byte *udata) {
	...
	for (;;) {
		st = inflateBack(&z, myin, nullptr, myout, udata);
		if (st == Z_STREAM_END)
			break;
		if (st)
			return st;
...
int sf_pkread(FILE *f, int foffs, void **out, int *size) {
	...
	if (usize <= 0)
		return -12;
	data = (byte *)malloc(usize);
	if (!data)
		return -13;
	cdata = (byte *)malloc(csize);
	if (!cdata) {
		free(data);
		return -14;
	}
