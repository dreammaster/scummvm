...
zbyte far *zmp = nullptr;
zbyte far *pcp = nullptr;

static FILE *story_fp = nullptr;
...
static undo_t *first_undo = nullptr, *last_undo = nullptr, *curr_undo = nullptr;
...
void init_setup(void) {
	...
	f_setup.blorb_file = nullptr;
	f_setup.story_file = nullptr;
	f_setup.story_name = nullptr;
	f_setup.story_base = nullptr;
	f_setup.script_name = nullptr;
	f_setup.command_name = nullptr;
	f_setup.save_name = nullptr;
	f_setup.tmp_save_name = nullptr;
	f_setup.aux_name = nullptr;
	f_setup.story_path = nullptr;
	f_setup.zcode_path = nullptr;
	f_setup.restricted_path = nullptr;
}
...
void init_memory(void) {
	...
	/* Open story file */
	if ((story_fp = os_load_story()) == nullptr)
		os_fatal("Cannot open story file");

	/* Allocate memory for story header */
	if ((zmp = (zbyte far *) malloc(64)) == nullptr)
...
no_match:; /* null statement */
	...
	if ((zmp = (zbyte far *) realloc(zmp, story_size)) == nullptr)
		os_fatal("Out of memory");
...
void init_undo(void) {
	 ...
	 */
	prev_zmp = (zbyte *)malloc(z_header.dynamic_size);
	undo_diff = (zbyte *)malloc(((unsigned long)z_header.dynamic_size * 3) / 2 + 2);
	if ((undo_diff != nullptr) && (prev_zmp != nullptr)) {
		memmove(prev_zmp, zmp, z_header.dynamic_size);
	} else {
		f_setup.undo_slots = 0;
		if (prev_zmp != nullptr) free(prev_zmp);
		if (undo_diff != nullptr) free(undo_diff);
	}
...
void z_restart(void) {
	...
	if (z_header.version != V6) {
		...
		SET_PC(pc);
	} else call(z_header.start_pc, 0, nullptr, 0);
...
static void get_default_name(char *default_name, size_t length, zword addr) {
	if (addr != 0) {
		...
		if (strchr(default_name, '.') == nullptr)
...
void z_restore(void) {
	...
	if (zargc != 0) {
		...
		/* Open auxilary file */
		if ((gfp = fopen(new_name, "rb")) == nullptr)
			goto finished;
...
finished:
	if (gfp == nullptr && f_setup.restore_mode)
		os_fatal("Error reading save file");
...
void z_save(void) {
	...
	if (zargc != 0) {
		...
		/* Open auxilary file */
		if ((gfp = fopen(new_name, "wb")) == nullptr)
			goto finished;
