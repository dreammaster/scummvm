...
void add_notify_request(LONG client, LONG message, LONG event, LONG
	parameter) {
	LONG i, nxt, cur = 0;
	NREQ *NR;
	...
	if (i == -1) {
		...
		out = open("event.dbg", ATF_WRITE);

		for (i = 0; i < NR_LSIZE; i++)
			fprintStr(out, "Message %5u, Client %5u, Next %5u, Prev %5u, Stat %5u, Parm %ld\n",
...
void delete_notify_request(LONG client, LONG message, LONG event,
	LONG parameter) {
	LONG nxt, cur, prev;
	LONG fnxt, fcur = 0;
	NREQ *NR;
	...
	do
	{
		...
		while (nxt != -1)                           // while not at end of chain
		{
			...
			if ((message != (LONG)-1) && (message != (LONG)NR->message))
				continue;
...
/*********************************************************/
EVENT *scan_event_range(LONG first_type, LONG last_type) {
	EVENT *EV;
	ULONG t;
...
void dump_event_queue() {
	...
	while ((e = fetch_event()) != NULL)
		if (e->type <= SYS_KEYDOWN)
			debug(1, "Event %s, parameter %ld\n", strs[e->type], e->parameter);
		else
			debug(1, "User event, parameter %ld\n", e->parameter);
}
