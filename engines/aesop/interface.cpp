...
/*********************************************************/
void add_region_event(LONG type, LONG owner) {
	WORD nxt;
	ULONG r;
	NREQ *NR;
...
static void timer_callback()   // Warning: called during IRQ 0
{
	EVENT *EV;
...
/*********************************************************/
static void mouse_event_handler(LONG px, LONG py) {
	static LONG entry = 0;
	WORD nxt;
	LONG r;
	NREQ *NR;
...
void init_interface() {
	...
	btn_left = last_left = false;
	btn_right = last_right = false;
