...
#ifndef DUNGEONCRAFT_SHARED_DRAWTILE
#define DUNGEONCRAFT_SHARED_DRAWTILE
...
struct COMBAT_SPRITE_DATA
{
  ...
  void Clear();
  void Init(PIC_DATA &data, BOOL path, CString prefix, int x1, int y1, int x2, int y2);
   // no path, forced dir
  void Init(PIC_DATA &data, CString prefix, int x1, int y1, PATH_DIR direction);
  // adds to AnimateList, for stationary animations
  void Init(PIC_DATA &data, CString prefix, int x1, int y1);
...
int HaveLineOfSight(int x0, int y0, int x1, int y1);
...
BOOL isObstical(int x, int y, int w, int h, BOOL CheckOccupants=TRUE);
BOOL ValidCoords(int y,int x);
BOOL findEmptyCell(int &x, int &y, int w, int h,PATH_DIR dir=PathBAD);
