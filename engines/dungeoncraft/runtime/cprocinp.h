...
#ifndef DUNGEONCRAFT_SHARED_PROCINPUT
#define DUNGEONCRAFT_SHARED_PROCINPUT
...
public:
  CProcessInput();
  ~CProcessInput() { if (local != NULL) delete local; local = NULL; }
  ...
  { 
    if (local != NULL) 
    { 
      local->AllowInput(allow); 
      if (!allow) local->ClearInput();
    }
  };
...
// WARNING *****  WARNING  ******
{
  ...
  TASK_CombatEndingTurn=66,

  TASK_CombatInRouteSpriteAnimating=67,
  TASK_CombatTimeDelayMsg=68,
  TASK_CombatRoundDelay=69,
  TASK_CombatOver=70,
  ...
  TASK_AskToUnspellDoor=114,  
  TASK_AskToBashOrUnspellDoor=115, 
  TASK_CombatSpellFinalStage=116,  
  TASK_DropCharFromParty=117,
  TASK_AskBeforeFleeMap=118,

  TASK_NextAvailableNumber=119
...
{
  TASKMSG_None=0,
  TASKMSG_CancelForwardMove=1, // Future move (after premove)
...
#ifdef TASKTRACE
  void             TaskTrace(char *id, int flags = 0);
  FILE *m_traceFile;
#endif
public:
  TASKLIST(void);
...
//  void CheckForDraw(void); // Call OnDraw if m_invalid;
  ...
  void InsertEvent(GameEvent *event, 
                   EventDisposition disposition = KeepEvent);
  void PopEvent(GameEvent *active);
...
//
#define INPUTQUEUE_MAX_ENTRIES 200  // That should be sufficient!
