...
void CMyMenu::ReverseVideo(void)
{
   ...
   defTextColor=Black; // black
}

void CMyMenu::NormalVideo(void)
{
   highlightText=Black; // black
   defTextColor=White; // white
}

void CMyMenu::WriteText(int x, int y, int item)
{
  if ((item < 0) || (item >= numItems))
  {
   ...
   if ( (!useActive) || (item != activeItem) )
     DrawFont(x,y, ptext, FALSE, shortcutIndex[item]);
   else 
     DrawFont(x,y, ptext, TRUE, shortcutIndex[item]);
}

void CMyMenu::WriteText(int x, int y, LPCSTR ptext)
{
   DrawFont(x,y, ptext, FALSE, -1);
}

void CMyMenu::DrawFont(int x, int y, LPCSTR ptext, BOOL highlight, int shortcutIdx)
{
   long int SrcFont;
   ...
   else
   {
     ...
     else if ((textColor == CombatYellow) || (textColor == Yellow))
       SrcFont = globalData.YellowFont;
     else if ((textColor == CombatRed) || (textColor == Red))
       SrcFont = globalData.RedFont;
     else if ((textColor == CombatGreen) || (textColor == Green))
       SrcFont = globalData.GreenFont;
     else if ((textColor == CombatBlue) || (textColor == Blue))
   ...
   GraphicsMgr.DrawFont(x,y, ptext, 
                        SrcFont, 
                        FALSE/*slow?*/, 
                        TRUE/*backbuffer?*/, 
                        TRUE/*transparent?*/, 
...
void CMyMenu::setStartCoord(const int x, const int y) 
{   
...
void CMyMenu::setMenu(MENU_DATA_TYPE& menuEnt)
{
  ...
  MapKeyCode=0;
  MapKeyCodeItem=-1;
  setStartCoord(menuEnt.x, menuEnt.y);
...
void CMyMenu::AddOverrideMenu(const char *ptext, const int shortcut, BOOL active)
{
  ...
  PrevStartX = startX;
  PrevStartY = startY;
  setStartCoord(DEFAULT_MENU_HORZ_X, DEFAULT_MENU_HORZ_Y);
  setHorzOrient();
  setUseActive(active);
  enableTitle(FALSE);
...
void CMyMenu::RemoveOverrideMenu()
{
  ...
  setStartCoord(PrevStartX, PrevStartY);
