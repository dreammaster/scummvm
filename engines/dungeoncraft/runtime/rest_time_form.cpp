...
static enum RESTTIME_FORM // Define field names as integers for quicker comparisons
{   
  RTF_none,
...
#define RTF(item,xrel,x,yrel,y) \
  { DISPLAY_FORM(RTF_##xrel,RTF_##yrel,(__int16)RTF_##item,x,y) },
...
{ 
   ...
   pForm->SetText(RTF_days,    temp, color);

   pForm->SetText(RTF_DAYSCOLON, ":", color);

   temp.Format("%02I64i", pData->m_hours);
   pForm->SetText(RTF_hours,   temp, color);

   pForm->SetText(RTF_HOURSCOLON, ":", color);

   temp.Format("%02I64i", pData->m_mins);
   pForm->SetText(RTF_minutes, temp, color);   
}
