...
#ifndef DUNGEONCRAFT_SHARED_GAMEMENU
#define DUNGEONCRAFT_SHARED_GAMEMENU
...
const int ChangeClassMenuItems=8;
...
private:
  ...
  int  getOrient() { return orient; }
	void WriteText(int x, int y, LPCSTR text);
	void WriteText(int x, int y, int item);
  void DrawFont(int x, int y, LPCSTR text, BOOL highlight, int shortcutIdx);
...
public:
  ...
  BOOL IsVertOrient() { return (orient == 1); }
	...
	void setStartCoord(const int x, const int y);
