...
//#include "dungeoncraft/shared/afxtempl.h"
    ...
    int *valueColumn)
{
  ...
  len=src.GetLength();
  temp=(char *) malloc(len+10); // Cannot get longer...only shorter
  if (temp==NULL)
  {
     ...
     CString& objectName)  // Returns name of object
{
     ...
     itemNumber++)  // For each line in this object
  {
    ...
    if (pkt.status>worstStatus) worstStatus=pkt.status;
...
bool CONFIG_FILE::Exists(int objectNumber)
{
...
CONFIG_FILE::OBJECTS::OBJECTS(void) {
  m_numberOfObjects=0;
  m_allocatedSize=10;
  m_objectList=(OBJECT **)malloc(m_allocatedSize*sizeof (*m_objectList));
...
int CONFIG_FILE::OBJECTS::AddObject (int objectNumber)
{
  ...
  if (objectNumber>=m_allocatedSize)
  {
    ...
    m_objectList=(OBJECT **)realloc(m_objectList, newSize*sizeof (*m_objectList));
    if (m_objectList==NULL)
    {
      MsgBoxError("OBJECTS memory allocation error");
      _exit(1);
    ...
    // valueColumn set to first character of value or -1.
{
  ...
  if ((helpMsgID!=NULL) || (errorMsg!=NULL))
  {
    ...
    pkt.func=CONFIG_FUNC_beginObject;
    for (int i=0; m_ids[i].m_id.m_dataType!=CONFIG_DATA_end;i++)
    {
...
CONFIG_FILE::OBJECTS::~OBJECTS(void) {
  ...
  if (m_objectList!=NULL) free (m_objectList);
}
...
CONFIG_FILE::OBJECT::OBJECT(void) {
  m_allocatedSize=10;
  m_numberOfItems=0;
  m_itemList=(LINE_ITEM **)malloc(m_allocatedSize*sizeof (*m_itemList));
  if (m_itemList==NULL)
  {
...
CONFIG_FILE::OBJECT::~OBJECT(void) 
{
  ...
  m_numberOfItems=0;
  if (m_itemList!=NULL) free(m_itemList);
  m_itemList=NULL;
  m_allocatedSize=0;
}
            ...
            const CString& value)
{
  ...
  if (m_numberOfItems>m_allocatedSize) 
  {
    ...
    m_itemList=(LINE_ITEM **)realloc(m_itemList,
    ...
    DWORD &nFlags)
{
  ...
  nFlags=6;  // loop once
	...
	// Expect value of form "filename,delay,frames,width,height,style"
	int incol = 0;
  status = GetStringUntilComma(incol,value, temp, errorMsg);
	if (status != CONFIG_STAT_ok) return status;
...
void CONFIG_ENCODE_MonsterAttack(
            ...
            MONSTER_ATTACK_DETAILS *data)
{
	// Produce value of form "sides,nbr,bonus,message"
