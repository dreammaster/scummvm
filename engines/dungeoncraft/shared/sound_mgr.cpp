...
#include "dungeoncraft/shared/Bass.h"
...
BOOL BACKGROUND_SOUNDS::operator ==(BACKGROUND_SOUNDS& src)
{
...
BOOL BACKGROUND_SOUND_DATA::operator ==(BACKGROUND_SOUND_DATA& src)
{
...
//
//*****************************************************************************
BOOL SoundQueue::Play(const char *filename)
{
  ...
  //WriteDebugString("SoundQueue playing %s\n", filename);
...
#ifdef UAFEngine
  CString filename;
  filename.Format("%ssound.ini", GetDesignPath());
...
long SoundMgr::AddSound(const char *filename)
{
  ...
  {
    WriteDebugString("Cant find file \'%s\' in AddSound()\n", tmp);
    return -1;
  }
...
{
  CString path;
  path.Format("%sbass.dll", m_installDir);
...
///////////////////////////////////////////////////////////////////////////////
#ifdef UAFEDITOR
CList<ResourceSaveType, ResourceSaveType&> LevelSoundData[MAX_LEVELS+1];
#endif
...
//
//*****************************************************************************
void saveSound(CString &filename, int level, BOOL alterPath)
{
#ifdef UAFEDITOR
  if (filename.IsEmpty())
    return;
...
BOOL writeSoundToFile(CString &filename)
{
  ...
  if (!FileExists(filename))
  {
    ...
    if (!SearchForFile(tmp)) // returns full path file
    {
      WriteDebugString("Cant find file \'%s\' in writeSoundToFile()\n", tmp);
      return FALSE;
    }
...
void WriteRequiredSoundList()
{
  ...
  GetDesignPath(dsnPath);
  CString filename;
  filename.Format("%sReqSnds.txt", dsnPath);
