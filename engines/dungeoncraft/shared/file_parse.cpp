...
BOOL FileParse::Parse(const char *filename, int SearchDepth)
{
  ...
  // if drive letter used, search by truncating path name
  // else search higher level folders
  BOOL UseAdd = (strstr(filename, ":\\") == NULL);

  // search current folder, and then higher folders up
  // to search depth
  while ((count < SearchDepth) && (!found))
  {
    if (!FileExists(path))
    {
      if (UseAdd)
      {
        temp.Format("%s%s", AddFolder, path);
        strcpy(path, temp);
      }
      else
      {
        ...
        // chop off next higher folder
        ptr = strrchr(path, '\\');
        if (ptr != NULL)
          *ptr = '\0';

        // add 'config.txt' again
