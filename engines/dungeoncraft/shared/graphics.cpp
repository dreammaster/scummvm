...
#include "dungeoncraft/shared/..\UAFWin\MouseThread.h"
...
#ifdef UAFEngine
#include "dungeoncraft/shared/Movie.h"
...
BOOL RunFullscreen=TRUE; // engine only, release build only
...
BOOL Graphics::AddMouseSurfaces(const char *filename)
{
  ...
  BOOL success = SearchForFile(file);
  int width = 32;
  int height = 32;
  int frames = 15;
...
//
//*****************************************************************************
long Graphics::AddSurface(const char *filename, SurfaceType type)
{
  ...
  {
    WriteDebugString("Cant find file in AddSurface(%s)\n", tmp);
    return -1;
  }
...
//
//*****************************************************************************
long Graphics::AddSprite(const char *filename, int width, int height, int frames, SurfaceType type)
{
  ...
  */
    if (!SearchForFile(tmp)) // returns full path file
    {
      WriteDebugString("Cant find file in AddSprite(%s)\n", tmp);
      return -1;
    }
  //}
  ...
  if (success)
    key = m_SpriteMgr.AddSurface(pSurf, type);
...
BOOL Graphics::InitGraphicsFullScreen(HWND hWnd, int &w, int &h, int &bits)
{
  ...
  WriteDebugString("Creating full screen()\n");
...
//
//*****************************************************************************
BOOL Graphics::DrawFont(int X, int Y, const char *Text, long SrcFontSurf, BOOL SlowText, BOOL UseBackBuffer, BOOL Transparent, int KeyIndex)
{
  if (m_pScreen == NULL) return FALSE;
  ...
  {   
    if (FontColorOn)
    {
      while (strnicmp(pText, "/h", 2) == 0) // could be /h/h/h in string
      {
        HighlightText(!HighlightOn);
        pText += 2;
      }
      while (strnicmp(pText, "/w", 2) == 0)
      {
        WhiteText(!WhiteOn);
        pText += 2;
      }
      while (strnicmp(pText, "/y", 2) == 0)
      {
        YellowText(!YellowOn);
        pText += 2;
      }
      while (strnicmp(pText, "/r", 2) == 0)
      {
        RedText(!RedOn);
        pText += 2;
      }
      while (strnicmp(pText, "/g", 2) == 0)
      {
        GreenText(!GreenOn);
        pText += 2;
      }
      while (strnicmp(pText, "/b", 2) == 0)
      {
        BlueText(!BlueOn);
        pText += 2;
      }
    
      while (strnicmp(pText, "/c", 2) == 0)
      {
...
//
//*****************************************************************************
BOOL Graphics::FadeInImage(const char *filename, SurfaceType type)
{
  if (m_pScreen == NULL) return FALSE;
  BOOL result = TRUE;
  long SplashBgDib = AddSurface(filename, type);
...
BOOL Graphics::BlitImage(WORD dstX, WORD dstY, long srcSurf, const RECT *srcRect, BOOL flipY, long dstSurf, const RECT *dstRect)
{
  ...
  else  // no transparency
  {
    ...
    else
    {
      ...
      {
        // dd bltfast
        while (((rc = pSrc->DrawBlk(pDst, dstX, dstY, const_cast<RECT*>(srcRect))) == DDERR_SURFACELOST) && (retries < 3))
        {
          if (!RestoreAll(m_pScreen, pDst, pSrc)) return FALSE;
          //pSrc->Restore();
          //pDst->Restore();
