...
#ifndef DUNGEONCRAFT_SHARED_PARTY
#define DUNGEONCRAFT_SHARED_PARTY
...
struct JOURNAL_DATA
{
  ...
  void Delete(JOURNAL_ENTRY &data)
  {
    ...
    while (pos != NULL)
    {
      if (jlist.GetAt(pos).entry == data.entry)
      {
        jlist.RemoveAt(pos);
        return;
      }
...
public:
  ...
  int GetInPartyCount();
  void SetInParty(int index, BOOL status);
  ...
  BOOL IsCharacterInTemps(CHARACTER &dude);
};
...
public:
  ...
  PARTY() :party_asl("PARTY_ATTRIBUTES") { Clear(TRUE); }
  ~PARTY() { Clear(TRUE); }
  ...
  BOOL newForwardPosition(int relativeDirection, int &newX, int &newY);
  ...
  BOOL CanPassBlockage(BlockageType type, int facing);
  ...
  BOOL PartyHasSpellMemorized(DWORD spellDbKey);
