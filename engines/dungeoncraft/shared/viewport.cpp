...
#include "dungeoncraft/shared/..\UAFWinEd\DlgPicture.h" // to allow config of mapping coords
#else
...
void ConfigureArtCoords()
{
  ...
  InitRect(ViewportBkgrnd, 0,0,ViewportWidth,ViewportHeight );
...
//
//*****************************************************************************
long getWallSurface(int facing, int i) // viewmap index
{
   ...
   ASL *pAsl = &globalData.levelInfo.stats[globalData.currLevel].level_asl;
   if (pAsl!=NULL)
   {
	   if (pAsl->Exists(temp)) 
	   {
   ...
   return WallSets[wallSlot].GetWallSurface();
}
...
//
//*****************************************************************************
long getDoorSurface(int facing, int i) // viewmap index
{
  if ((i<0) || (i>14))  return -1;
  ...
  if (wallSlot==0) return -1;
  return WallSets[wallSlot].GetDoorSurface();
}
...
//
//*****************************************************************************
long getOverlaySurface(int facing, int i) // viewmap index
{
  if ((i<0) || (i>14))  return -1;
  if (!validCoords(viewMap[i].y,viewMap[i].x)) return -1;
  BYTE wallSlot=levelData.area[viewMap[i].y][viewMap[i].x].walls(facing);
  ...
  if (wallSlot==0) return -1;
  return WallSets[wallSlot].GetOverlaySurface();
}
...
//
//*****************************************************************************
WORD getWallSlotFormat(int facing, int i) // viewmap index
{
  if ((i<0) || (i>14))  return DEFAULT_FORMAT_TYPE;
  if (!validCoords(viewMap[i].y,viewMap[i].x)) return DEFAULT_FORMAT_TYPE;
  BYTE wallSlot=levelData.area[viewMap[i].y][viewMap[i].x].walls(facing);
...
//
//*****************************************************************************
int GetWallASL(int x, int y, int facing) 
{
  ...
  if (wallSlot==0) return -1;
  if (WallSets[wallSlot].drawAV) return -1;
  else return 0; // this wall slot is not included in area view
}
...
void updateAreaView(int plyrX, int plyrY, int facing, long int surface)
{
   ...
   if (!LoadAreaViewArt(globalData.MapArt, globalData.MapArtSurf, globalData.UseAVZones))
   {
...
#ifdef UAFEngine
           if (EditorRunning) // only true while testing design from editor
#endif
           {
...
#ifdef UAFEngine
               if (EditorRunning) // only true while testing design from editor, or running engine _DEBUG build
#endif
               {
                 // display event markers if in debug mode
                 if (levelData.haveEvent(j,i))
...
void RenderSquare0(int left, int facing, int right, long dstSurface)
{
  ...
  WORD format;
  int p_width=0;  
  format = getWallSlotFormat(facing,0);
  ...
  if (WallCount==5)
  {
    srcSurface = getWallSurface(facing,0);
    p_width = WallFormatMgr.GetWidth(format, DRAW_P_WALL);

    if (srcSurface > 0)
    {           
     if (   (getWallSurface(facing,13) > 0) 
         || (getWallSurface(left,0) > 0)
         || (getWallSurface(right,13) > 0)
    ...
    if (!doorFirst)
    {
      srcSurface = getOverlaySurface(facing,0);
      if (srcSurface > 0)
      {
       if (   (getWallSurface(facing,13) > 0) 
           || (getWallSurface(left,0) > 0)
      ...
      srcSurface = getDoorSurface(facing,0);            
      if (srcSurface > 0)
        BltSurface(pt.x+p_width, pt.y, DRAW_J_WALL, srcSurface, dstSurface,format);
    }
    else
...
void RenderSquare1(int left, int facing, int right, long dstSurface)
{
  ...
  if (WallCount==5)
  {
    srcSurface = getWallSurface(facing,1);
    p_width = WallFormatMgr.GetWidth(format, DRAW_P_WALL);

    if (srcSurface > 0)
    {
     BltSurface(pt.x, pt.y, DRAW_P_WALL, srcSurface, dstSurface,format);
     if (   (getWallSurface(facing,14) > 0) 
         || (getWallSurface(right,1) > 0)
         || (getWallSurface(left,14) > 0)
    ...
    if (!doorFirst)
    {
      srcSurface = getOverlaySurface(facing,1);
      if (srcSurface > 0)
      {
      ...
      srcSurface = getDoorSurface(facing,1);            
      if (srcSurface > 0)
       BltSurface(pt.x, pt.y, DRAW_P_WALL, srcSurface, dstSurface,format);
    }
    else
...
void RenderSquare2(int left, int facing, int right, long dstSurface)
{
  ...
  if (WallCount==5)
  {
    srcSurface = getWallSurface(facing,2);
    p_width = WallFormatMgr.GetWidth(format, DRAW_P_WALL);
    j_width = WallFormatMgr.GetWidth(format, DRAW_J_WALL);

    if (srcSurface > 0)
    {
     if ((getWallSurface(facing,0) > 0) || (getWallSurface(left,2) > 0))
    ...
    if (!doorFirst)
    {
      srcSurface = getOverlaySurface(facing,2);
      if (srcSurface > 0)
      {
       if ((getWallSurface(facing,0) > 0) || (getWallSurface(left,2) > 0))
      ...
      srcSurface = getDoorSurface(facing,2);            
      if (srcSurface > 0)
       BltSurface(pt.x+p_width, pt.y, DRAW_J_WALL, srcSurface, dstSurface,format);
    }
    else
...
void RenderSquare3(int left, int facing, int right, long dstSurface)
{
  ...
  if (WallCount==5)
  {
    srcSurface = getWallSurface(facing,3);
    j_width = WallFormatMgr.GetWidth(format, DRAW_J_WALL);

    if (srcSurface > 0)
    {
     BltSurface(pt.x, pt.y, DRAW_J_WALL, srcSurface, dstSurface,format);
     if ((getWallSurface(facing,1) > 0) || (getWallSurface(right,3) > 0))
       BltSurface(pt.x+j_width, pt.y, DRAW_P_WALL, srcSurface, dstSurface,format);
    }

    if (!doorFirst)
    {
      srcSurface = getOverlaySurface(facing,3);
      if (srcSurface > 0)
      {
      ...
      srcSurface = getDoorSurface(facing,3);
      if (srcSurface > 0)
       BltSurface(pt.x, pt.y, DRAW_J_WALL, srcSurface, dstSurface,format);
    }
    else
...
void RenderSquare4(int left, int facing, int right, long dstSurface)
{
  ...
  if (WallCount==5)
  {
    srcSurface = getWallSurface(facing,4);
    j_width = WallFormatMgr.GetWidth(format, DRAW_J_WALL);

    if (srcSurface > 0)
    {
     if (getWallSurface(facing,2) > 0)
       BltSurface(pt.x, pt.y, DRAW_J_WALL, srcSurface, dstSurface,format);
     BltSurface(pt.x+j_width, pt.y, DRAW_P_WALL, srcSurface, dstSurface,format);
     if (getWallSurface(facing,3) > 0)
       BltSurface(pt.x+(2*j_width), pt.y, DRAW_J_WALL, srcSurface, dstSurface,format);
    }

    if (!doorFirst)
    {
      srcSurface = getOverlaySurface(facing,4);
      if (srcSurface > 0)
      {
       if (getWallSurface(facing,2) > 0)
      ...
      srcSurface = getDoorSurface(facing,4);            
      if (srcSurface > 0)
...
void RenderSquare8(int left, int facing, int right, long dstSurface)
{
  ...
  srcSurface = getWallSurface(facing,8);
  if (srcSurface > 0)
   BltSurface(pt.x, pt.y, DRAW_H_WALL, srcSurface, dstSurface,format);

  if (!doorFirst)
  {
    srcSurface = getOverlaySurface(facing,8);
    if (srcSurface > 0)
     BltOverlaySurface(pt.x, pt.y, DRAW_H_WALL, srcSurface, dstSurface,format,getWallSlot(facing,8));

    srcSurface = getDoorSurface(facing,8);            
    if (srcSurface > 0)
     BltSurface(pt.x, pt.y, DRAW_H_WALL, srcSurface, dstSurface,format);
  }
  else
...
void RenderSquare9(int left, int facing, int right, long dstSurface)
{
  ...
  srcSurface = getWallSurface(facing,9);
  if (srcSurface > 0)
   BltSurface(pt.x, pt.y, DRAW_H_WALL, srcSurface, dstSurface,format);

  if (!doorFirst)
  {
    srcSurface = getOverlaySurface(facing,9);
    if (srcSurface > 0)
     BltOverlaySurface(pt.x, pt.y, DRAW_H_WALL, srcSurface, dstSurface,format,getWallSlot(facing,9));

    srcSurface = getDoorSurface(facing,9);            
    if (srcSurface > 0)
     BltSurface(pt.x, pt.y, DRAW_H_WALL, srcSurface, dstSurface,format);
  }
  else
...
void RenderSquare13(int left, int facing, int right, long dstSurface)
{
  ...
  srcSurface = getWallSurface(facing,13);
  if (srcSurface > 0)
    BltSurface(pt.x, pt.y, DRAW_J_WALL, srcSurface, dstSurface, format);

  if (!doorFirst)
  {
    srcSurface = getOverlaySurface(facing,13);
    if (srcSurface > 0)
      BltOverlaySurface(pt.x, pt.y, DRAW_J_WALL, srcSurface, dstSurface,format,getWallSlot(facing,13));

    srcSurface = getDoorSurface(facing,13);
    if (srcSurface > 0)
      BltSurface(pt.x, pt.y, DRAW_J_WALL, srcSurface, dstSurface,format);
  }
  else
...
void RenderSquare14(int left, int facing, int right, long dstSurface)
{
  ...
  srcSurface = getBackgroundSurface(facing,14);
  if (srcSurface > 0)
    BltSurface(pt.x, pt.y, DRAW_J_WALL, srcSurface, dstSurface, format);

  if (!doorFirst)
  {
    srcSurface = getOverlaySurface(facing,14);
    if (srcSurface > 0)
      BltOverlaySurface(pt.x, pt.y, DRAW_J_WALL, srcSurface, dstSurface,format,getWallSlot(facing,14));

    srcSurface = getDoorSurface(facing,14);
    if (srcSurface > 0)
      BltSurface(pt.x, pt.y, DRAW_J_WALL, srcSurface, dstSurface,format);
  }
  else
...
void RenderWalls(int facing, long int dstSurface)
{
  ...
  for (int i=0; i<13; i++)
  {
    switch (i) 
    {
    case 0: RenderSquare0(left, facing, right, dstSurface); break;
    case 1: RenderSquare1(left, facing, right, dstSurface); break;
    case 2: RenderSquare2(left, facing, right, dstSurface); break;
    case 3: RenderSquare3(left, facing, right, dstSurface); break;
    case 4: RenderSquare4(left, facing, right, dstSurface); break;
    case 5: RenderSquare5(left, facing, right, dstSurface); break;
    case 6: RenderSquare6(left, facing, right, dstSurface); break;
    case 7: RenderSquare7(left, facing, right, dstSurface); break;
    case 8: RenderSquare8(left, facing, right, dstSurface); break;
    case 9: RenderSquare9(left, facing, right, dstSurface); break;
    case 10:RenderSquare10(left, facing, right, dstSurface); break;
    case 11:RenderSquare11(left, facing, right, dstSurface); break;
    case 12:RenderSquare12(left, facing, right, dstSurface); break;
    }
  }
}
...
void updateViewport(int plyrX, int plyrY, int facing, long int dstSurface)
{
...
//                to the back buffer
///////////////////////////////////////////////////////////////////////
void drawViewport(BOOL drawMiddleVertBar, BOOL drawFrame)
{
...
void drawBorder(void)
{
   ...
   GraphicsMgr.BlitImage( VertBarRightX, VertBarRightY, VBarVPArt.surface, &VertBarSrc );
