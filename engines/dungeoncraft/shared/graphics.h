...
#ifndef DUNGEONCRAFT_SHARED_GRAPHICS
#define DUNGEONCRAFT_SHARED_GRAPHICS
...
#ifdef UAFEngine 
#include "dungeoncraft/shared/..\UAFWin\CDXBitmapFont.h"
#endif
...
#define LOGERROR(pScreen, pErrorStr) {WriteDebugString("%i: %s", __LINE__, pErrorStr); SetMiscError(GraphicEngineError);}
#endif
...
public:
  ...
  long AddSurface(const char *filename, SurfaceType type);
  ...
  void Custom1Text(BOOL enable);

  // blits created font from font surface
  BOOL DrawFont(int X, int Y, const char *Text, long int SrcFontSurf, BOOL SlowText=FALSE, BOOL UseBackBuffer=TRUE, BOOL Transparent=TRUE,int KeyIndex=-1);
  ...
  BOOL FadeInImage(const char *filename, SurfaceType type);
