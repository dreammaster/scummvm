...
#include "dungeoncraft/shared/stdafx.h"
#include <math.h>
...
//
//*****************************************************************************
BOOL MONEY_SACK::operator==(const MONEY_SACK &rhs)
{
  for (int i=0;i<NumCoinTypes();i++)
    if (Coins[i] != rhs.Coins[i]) return FALSE;
  ...
  while (p1!=NULL)
  {
    ...
    if (!(Gems.GetNext(p1) == const_cast<MONEY_SACK&>(rhs).Gems.GetNext(p2))) return false;
  }
  if (p2!=NULL) return false;
  ...
  {
    if (p2==NULL) return false;
    if (!(Jewelry.GetNext(p1)==const_cast<MONEY_SACK&>(rhs).Jewelry.GetNext(p2))) return false;
  }
  if (p2!=NULL) return false;
...
//
//*****************************************************************************
void MONEY_SACK::Add(MONEY_SACK &src)
{
  for (int i=0;i<NumCoinTypes();i++)
    Coins[i] += src.Coins[i];
...
//
//*****************************************************************************
void MONEY_SACK::Subtract(MONEY_SACK &src)
{
  for (int i=0;i<NumCoinTypes();i++)
...
//
//*****************************************************************************
int MONEY_SACK::AddGem(GEM_TYPE &data)
{
  if (Gems.GetCount() >= MAX_GEMS)
    return 0;
...
//
//*****************************************************************************
BOOL MONEY_SACK::AddGems(GEM_LIST &data)
{
...
//
//*****************************************************************************
int MONEY_SACK::AddJewelry(GEM_TYPE &data)
{
  if (Jewelry.GetCount() >= MAX_JEWELRY)
    return 0;
...
//
//*****************************************************************************
BOOL MONEY_SACK::AddJewelry(JEWELRY_LIST &data)
{
  POSITION pos = data.GetHeadPosition();
  while (pos != NULL)
  {
    if (AddJewelry(data.GetNext(pos)) == 0)
      return FALSE; // could not add
  }
