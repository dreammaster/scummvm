...
KeyInput ags_keycode_from_scummvm(const Common::Event &event, bool old_keyhandle) {
	...
	ki.Key = ::AGS2::g_events->scummvm_key_to_ags_key(event, ki.Mod, old_keyhandle);
	ki.CompatKey = ::AGS2::g_events->scummvm_key_to_ags_key(event, ki.Mod, true);
...
bool ags_keyevent_ready() {
	return ::AGS2::g_events->keyEventPending();
}

Common::Event ags_get_next_keyevent() {
	return ::AGS2::g_events->getPendingKeyEvent();
}

int ags_iskeydown(eAGSKeyCode ags_key) {
	return ::AGS2::g_events->isKeyPressed(ags_key, _GP(game).options[OPT_KEYHANDLEAPI] == 0);
}

void ags_simulate_keypress(eAGSKeyCode ags_key, bool old_keyhandle) {
	Common::KeyCode keycode[3];
	if (!::AGS2::EventsManager::ags_key_to_scancode(ags_key, keycode))
		return;
	...
	::AGS2::g_events->pushKeyboardEvent(e);
...
void sys_evt_process_pending(void) {
	...
	while ((e = ::AGS2::g_events->readEvent()).type != Common::EVENT_INVALID)
		sys_process_event(e);
}
