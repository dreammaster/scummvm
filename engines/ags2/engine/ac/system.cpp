...
int System_GetNumLock() {
	return (::AGS2::g_events->getModifierFlags() & Common::KBD_NUM) ? 1 : 0;
}

int System_GetCapsLock() {
	return (::AGS2::g_events->getModifierFlags() & Common::KBD_CAPS) ? 1 : 0;
}

int System_GetScrollLock() {
	return (::AGS2::g_events->getModifierFlags() & Common::KBD_SCRL) ? 1 : 0;
}
...
void System_SetVolume(int newvol) {
	...
	Audio::Mixer *mixer = ::AGS2::g_vm->_mixer;
