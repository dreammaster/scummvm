...
#include "ags2/shared/core/asset.h"
...
bool ResolveScriptPath(const String &orig_sc_path, bool read_only, ResolvedPath &rp) {
	...
	if (sc_path.CompareLeft(GameInstallRootToken) == 0) {
		...
		if (child_path.CompareLeft(agsSavePrefix) == 0) {
			debugC(::AGS2::kDebugFilePath, "Remapping agssave.* to ScummVM savegame files");
			String suffix = child_path.Mid(strlen(agsSavePrefix));
			if (suffix.CompareLeft("*") == 0) {
				Common::String file_name = ::AGS2::g_vm->getSaveStateName(999);
				Common::replace(file_name, "999", "*");
				child_path = file_name;
			} else {
				int slotNum = suffix.ToInt();
				child_path = ::AGS2::g_vm->getSaveStateName(slotNum);
			}
		}
#endif
		...
		debugC(::AGS2::kDebugFilePath, "Remapping %s to save folder", GameDataDirToken);
		parent_dir = get_save_game_directory();
		child_path = sc_path.Mid(strlen(GameSavedgamesDirToken));
#else
	...
	if (parent_dir.BaseDir == SAVE_FOLDER_PREFIX) {
		debugC(::AGS2::kDebugFilePath, "Adding ScummVM game target prefix and flatten path");
		child_path.Replace('/', '-');
	...
	rp = test_rp;
	debugC(::AGS2::kDebugFilePath, "Final path: %s", rp.FullPath.GetCStr());
	if (!rp.AltPath.IsEmpty())
		debugC(::AGS2::kDebugFilePath, "Alt path: %s", rp.AltPath.GetCStr());
	return true;
}
