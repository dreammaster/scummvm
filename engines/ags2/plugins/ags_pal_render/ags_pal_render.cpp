...
#include "ags2/plugins/ags_pal_render/pal_render.h"
#include "ags2/plugins/ags_pal_render/raycast.h"
#include "ags2/ags2.h"
...
void AGSPalRender::DoFire(ScriptMethodParams &params) {
	...
	for (int y = 0, firey = 0; y < h - 1; y++, firey += firePitch) {
		if ((int)::AGS2::g_vm->getRandomNumber(9) > 7 - windspeed) { //Wind right
			for (int x = w - 1; x > 1; x--) {
				fire[firey + x] = fire[firey + x - 1];
			}
		} else if ((int)::AGS2::g_vm->getRandomNumber(9) > 7 + windspeed) { // wind left
			for (int x = 0; x < w - 1; x++) {
				fire[firey + x] = fire[firey + x + 1];
			}
		}
	...
	for (int x = 0; x < w; x++) {
		sparky = ABS((int)::AGS2::g_vm->getRandomNumber(0x7fffffff) % (h - 2));
		int firexy = (h - sparky) * firePitch + x;
		if (sparky < h && sparky > 0 && fire[firexy] > cutoff &&
				ABS((int)::AGS2::g_vm->getRandomNumber(0x7fffffff) % 10) > 7)
			fire[firexy] = 255;
		sparky = ABS((int)::AGS2::g_vm->getRandomNumber(0x7fffffff) % (h - 2));
	...
	if (seed == 0) {
		...
		firey = (h - 2) * firePitch;
		for (int x = 0; x < w; x++)
			fire[firey + x] = ::AGS2::g_vm->getRandomNumber(255);
	} else if (seed > 0) {
...
void AGSPalRender::InitializeStars(ScriptMethodParams &params) {
	...
	for (int i = 0; i < Starfield.maxstars; i++) {
		stars[i].x = (float)((::AGS2::g_vm->getRandomNumber(0x7fffffff) % sw) << 1) - sw;
		if (stars[i].x < 1.0 && stars[i].x > -1.0) stars[i].x = (float)sw;
		stars[i].y = (float)((::AGS2::g_vm->getRandomNumber(0x7fffffff) % sh) << 1) - sh;
		if (stars[i].y < 1.0 && stars[i].y > -1.0) stars[i].y = (float)sh;
		stars[i].z = (float)(MAX_DEPTH);
		stars[i].color = (::AGS2::g_vm->getRandomNumber(0x7fffffff) % 240);
		stars[i].sprite = 0;
		stars[i].maxrad = (::AGS2::g_vm->getRandomNumber(0x7fffffff) % 5);
	}
}
