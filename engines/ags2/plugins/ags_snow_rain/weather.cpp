...
#include "ags2/plugins/ags_snow_rain/weather.h"
...
void Weather::Update() {
	...
	for (i = 0; i < _mAmount * 2; i++) {
		...
		if (_mParticles[i].y > _mParticles[i].max_y) {
			_mParticles[i].y = -1 * (::AGS2::g_vm->getRandomNumber(0x7fffffff) % _screenHeight);
			_mParticles[i].x = ::AGS2::g_vm->getRandomNumber(0x7fffffff) % _screenWidth;
			_mParticles[i].alpha = ::AGS2::g_vm->getRandomNumber(0x7fffffff) % _mDeltaAlpha + _mMinAlpha;
			_mParticles[i].speed = (float)(::AGS2::g_vm->getRandomNumber(0x7fffffff) % _mDeltaFallSpeed + _mMinFallSpeed) / 50.0f;
			_mParticles[i].max_y = ::AGS2::g_vm->getRandomNumber(0x7fffffff) % _mDeltaBaseline + _mTopBaseline;
...
void Weather::UpdateWithDrift() {
	...
	for (i = 0; i < _mAmount * 2; i++) {
		...
		if (_mParticles[i].y > _mParticles[i].max_y) {
			...
			_mParticles[i].speed = (float)(::AGS2::g_vm->getRandomNumber(0x7fffffff) % _mDeltaFallSpeed + _mMinFallSpeed) / 50.0f;
			_mParticles[i].max_y = ::AGS2::g_vm->getRandomNumber(0x7fffffff) % _mDeltaBaseline + _mTopBaseline;
			_mParticles[i].drift = ::AGS2::g_vm->getRandomNumber(0x7fffffff) % _mDeltaDrift + _mMinDrift;
			_mParticles[i].drift_speed = (::AGS2::g_vm->getRandomNumber(0x7fffffff) % _mDeltaDriftSpeed + _mMinDriftSpeed) / 50.0f;
		} else if ((_mParticles[i].y > 0) && (_mParticles[i].alpha > 0))
...
void Weather::InitializeParticles() {
	for (uint i = 0; i < ARRAYSIZE(_mParticles); i++) {
		_mParticles[i].clear();
		_mParticles[i].kind_id = ::AGS2::g_vm->getRandomNumber(0x7fffffff) % 5;
		_mParticles[i].y = ::AGS2::g_vm->getRandomNumber(0x7fffffff) % (_screenHeight * 2) - _screenHeight;
		_mParticles[i].x = ::AGS2::g_vm->getRandomNumber(0x7fffffff) % _screenWidth;
		_mParticles[i].alpha = ::AGS2::g_vm->getRandomNumber(0x7fffffff) % _mDeltaAlpha + _mMinAlpha;
		_mParticles[i].speed = (float)(::AGS2::g_vm->getRandomNumber(0x7fffffff) % _mDeltaFallSpeed + _mMinFallSpeed) / 50.0f;
		_mParticles[i].max_y = ::AGS2::g_vm->getRandomNumber(0x7fffffff) % _mDeltaBaseline + _mTopBaseline;
		_mParticles[i].drift = ::AGS2::g_vm->getRandomNumber(0x7fffffff) % _mDeltaDrift + _mMinDrift;
		_mParticles[i].drift_speed = (::AGS2::g_vm->getRandomNumber(0x7fffffff) % _mDeltaDriftSpeed + _mMinDriftSpeed) / 50.0f;
		_mParticles[i].drift_offset = ::AGS2::g_vm->getRandomNumber(0x7fffffff) % 100;
	}
}
