...
int cunpackbitl(unsigned char *, int size, Common::SeekableReadStream *infile);
...
void CustomPropertySchema::Serialize(Common::WriteStream *outto) {
...
int CustomPropertySchema::UnSerialize (Common::SeekableReadStream *infrom) {
  if (getw(infrom) != 1)
    return -1;
...
int CustomProperties::UnSerialize (Common::SeekableReadStream *infrom) {
  if (getw(infrom) != 1)
    return -1;
  numProps = getw(infrom);
  for (int ee = 0; ee < numProps; ee++) {
...
NewInteractionCommandList *deserialize_command_list (Common::SeekableReadStream *ooo) {
...
NewInteraction *deserialize_new_interaction (Common::SeekableReadStream *ooo) {
  int a;
...
long loadcompressed_allegro(Common::SeekableReadStream *fpp, BITMAP **bimpp, color *pall, long ooo) {
  short widd,hitt;
  int   ii;
...
extern void load_script_configuration(Common::SeekableReadStream *);
...
void read_string_decrypt(Common::SeekableReadStream *ooo, char *sss) {
  int newlen = getw(ooo);
...
void load_room(char *files, roomstruct *rstruc, bool gameIsHighRes) {
  ...
  int   thisblock = 0;
  long  bloklen;
  Common::SeekableReadStream *optywas;
...
void ViewStruct::ReadFromFile(Common::SeekableReadStream *iii)
{
#ifdef ALLEGRO_BIG_ENDIAN
...
void ConvertOldCharacterToNew (OldCharacterInfo *oci, CharacterInfo *ci) {
  ...
  COPY_CHAR_VAR (acty);
  COPY_CHAR_VAR (on);
  Common::strcpy_s (ci->name, oci->name);
  Common::strcpy_s (ci->scrname, oci->scrname);
...
void ConvertOldGameStruct (OldGameSetupStruct *ogss, GameSetupStruct *gss) {
  int i;
  Common::strcpy_s (gss->gamename, ogss->gamename);
  for (i = 0; i < 20; i++)
