...
short getshort(Common::SeekableReadStream *fff) {
	return fff->readSint16LE();
}

void putshort(short num, Common::WriteStream *fff) {
	fff->writeSint16LE(num);
}

block wloadblock(const char *fill) {
	short widd, hitt;
	Common::File fff;
	int ff;
	...
	for (ff = 0; ff < hitt; ff++)
		fff.read(&tempbitm->line[ff][0], widd);
...
int wloadsprites(color *pall, char *filnam, block *sarray, int strt, int eend) {
	...
	for (vv = 0; vv <= numspri; vv++) {
		...
		for (hh = 0; hh < htt; hh++)
			ff.read(&sarray[vv]->line[hh][0], wdd * coldep);
	}
...
void gettime(struct time *tpt) {
	...
	tpt->ti_sec = td.tm_sec;
	tpt->ti_min = td.tm_min;
	tpt->ti_hour = td.tm_hour;
	tpt->ti_hund = 0;
}
