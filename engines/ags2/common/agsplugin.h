...
public:
	...
	AGSIFUNC(void) DrawText(int x, int y, int font, int color, char *text);
	...
	AGSIFUNC(int)  FWrite(void *, int, intptr);
	// similar to fread - buffer, size, filehandle
	AGSIFUNC(int)  FRead(void *, int, intptr);
  ...
  // get reference to main DirectSound interface
	...
	AGSIFUNC(int)    CallGameScriptFunction(const char *name, int globalScript, int numArgs, int arg1 = 0, int arg2 = 0, int arg3 = 0);
	...
	AGSIFUNC(void)   QueueGameScriptFunction(const char *name, int globalScript, int numArgs, int arg1 = 0, int arg2 = 0);
