...
void SpriteCache::removeOldest() {
	...
	if ((images[sprnum] != NULL) && (offsets[sprnum] != SPRITE_LOCKED)) {
		...
		{
			char msgg[150];
			Common::sprintf_s(msgg, "SpriteCache::removeOldest: Attempted to remove sprite %d that does not exist", sprnum);
			quit(msgg);
		}
	...
	if (liststart == listend)
	{
		...
		{
			char msgg[150];
			Common::sprintf_s(msgg, "!!!! SPRITE CACHE ERROR: Sprite cache should be empty, but still has %d bytes", cachesize);
			write_log(msgg);
		}
		mrulist[liststart] = 0;
		liststart = -1;
	...
	} else
	{
		...
		if (oldstart == liststart) {
			...
			char msgg[150];
			Common::sprintf_s(msgg, "!!!! At size %ld (of %ld), start %d end %d  fwdlink=%d",
	...
	char msgg[100];
	Common::sprintf_s(msgg, "Removed %d, size now %d KB", sprnum, cachesize / 1024);
	write_log(msgg);
#endif
}
...
#ifdef DEBUG_SPRITECACHE
	char msgg[100];
	Common::sprintf_s(msgg, "Precached %d", index);
	write_log(msgg);
#endif
}
...
void SpriteCache::seekToSprite(int index) {
	if (index - 1 != lastLoad)
		ff->seek(offsets[index], SEEK_SET);
}
...
int SpriteCache::loadSprite(int index) {
	...
	if (this->spritesAreCompressed)
	{
		...
		for (hh = 0; hh < htt; hh++)
			...
			ff->read(&images[index]->line[hh][0], coldep * wdd);
	}
...
#ifdef DEBUG_SPRITECACHE
	char msgg[100];
	Common::sprintf_s(msgg, "Loaded %d, size now %d KB", index, cachesize / 1024);
	write_log(msgg);
#endif
...
void SpriteCache::compressSprite(block sprite, Common::WriteStream *ooo) {
...
int SpriteCache::saveToFile(const char *filnam, int lastElement, bool compressOutput) {
	...
	if (compressOutput) {
		...
		if (output == NULL)
			return -1;
#else
		error("TODO: saveToFile");
#endif
	}

	uint spriteFileIDCheck = g_system->getMillis();

	// version 6
	putshort(6, &output);
	output.write(spriteFileSig, strlen(spriteFileSig));
	output.writeByte(compressOutput ? 1 : 0);
	...
	for (i = 0; i <= lastslot; i++) {
		...
		output.writeStream(ff, sizeToCopy);
	}
	...
	Common::DumpFile ooo;
	if (!ooo.open(spindexfilename))
		error("Error writing sprite index");

	// write "SPRINDEX" id
	ooo.write(&spindexid[0], strlen(spindexid));
	...
	for (i = 0; i < numsprits; ++i)
		ooo.writeUint16LE(spritewidths[i]);
	for (i = 0; i < numsprits; ++i)
		ooo.writeUint16LE(spriteheights[i]);
	for (i = 0; i < numsprits; ++i)
		ooo.writeUint32LE(spriteoffs[i]);
	ooo.close();
...
int SpriteCache::initFile(const char *filnam) {
	...
	ff = clibfopen(filnam, "rb");
	if (ff == NULL)
		return -1;

	spr_initial_offs = ff->pos();
	vers = ff->readUint16LE();
	...
	if (vers == 4)
		...
		this->spritesAreCompressed = (ff->readByte() == 1);
		spriteFileID = getw(ff);
	}

	if (vers < 5) {
		// skip the palette
		ff->seek(256 * 3, SEEK_CUR);
	}

	numspri = ff->readUint16LE();
	...
	error("TODO: unlink(spindexfilename)");
	...
	for (vv = 0; vv <= numspri; vv++) {
		...
		ff->seek(spriteDataSize, SEEK_CUR);
	}
...
bool SpriteCache::loadSpriteIndexFile(int expectedFileID, long spr_initial_offs, short numspri) {
	...
	int vv;
	Common::SeekableReadStream *fidx = clibfopen(spindexfilename, "rb");
	if (fidx == NULL) {
		return false;
	}
	...
	char buffer[9];
	// check "SPRINDEX" id
	fidx->read(&buffer[0], strlen(spindexid));
	buffer[8] = 0;
	...
	int i;
	for (i = 0; i < numsprits; ++i)
		rspritewidths[i] = fidx->readUint16LE();
	for (i = 0; i < numsprits; ++i)
		rspriteheights[i] = fidx->readUint16LE();
	for (i = 0; i < numsprits; ++i)
		offsets[i] = fidx->readUint32LE();
