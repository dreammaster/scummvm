...
#include "ags2/shared/core/def_version.h"
...
#include "ags2/engine/game/savegame_components.h"
#include "ags2/engine/main/config.h"
#include "ags2/engine/main/engine.h"
#include "ags2/engine/main/main.h"
#include "ags2/engine/main/quit.h"
...
#include "ags2/engine/ac/route_finder.h"
...
AGSEngine::AGSEngine(OSystem *syst, const AGS2GameDescription *gameDesc) : Engine(syst),
	...
	AGS2::script_commands_init();
	AGS2::Engine::SavegameComponents::component_handlers_init();
	_events = new EventsManager();
	_globals = new ::AGS2::Globals();
...
AGSEngine::~AGSEngine() {
	...
	delete _music;
	delete _globals;
	AGS2::Engine::SavegameComponents::component_handlers_free();
	AGS2::script_commands_free();
}
...
Common::Error AGSEngine::run() {
	...
	const int ARGC = 2;
	AGS2::main_init(ARGC, ARGV);

	_G(debug_flags) = 0;

	if (ConfMan.hasKey("display_fps"))
		_G(display_fps) = ConfMan.getBool("display_fps") ? AGS2::kFPS_Forced : AGS2::kFPS_Hide;
	...
	AGS2::ConfigTree startup_opts;
	int res = AGS2::main_process_cmdline(startup_opts, ARGC, ARGV);
	if (res != 0)
		return Common::kUnknownError;

	if (_G(justDisplayVersion)) {
		_G(platform)->WriteStdOut(AGS2::get_engine_string().GetCStr());
		return Common::kNoError;
	}

	if (_G(justDisplayHelp)) {
		AGS2::main_print_help();
		return Common::kNoError;
	}

	if (!_G(justTellInfo))
		_G(platform)->SetGUIMode(true);
	AGS2::init_debug(startup_opts, _G(justTellInfo));
	AGS2::AGS::Shared::Debug::Printf("%s", AGS2::get_engine_string().GetCStr());

	AGS2::main_set_gamedir(ARGC, ARGV);
	...
	syncSoundSettings();
	AGS2::initialize_engine(startup_opts);
