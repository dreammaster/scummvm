...
void Character_LockView(CharacterInfo *chap, int vii) {
  ...
  if ((vii < 1) || (vii > game.numviews)) {
    char buffer[150];
    Common::sprintf_s(buffer, "!SetCharacterView: invalid view number (You said %d, max is %d)", vii, game.numviews);
    quit(buffer);
  }
  vii--;
