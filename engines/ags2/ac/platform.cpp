...
inline void *LoadLibrary(const char *handle) { return (void *)42; }
inline void *GetProcAddress(void *handle, const char *method) {
...
void AGSPlatformDriver::ReplaceSpecialPaths(const char *sourcePath, char *destPath) {
	if (scumm_strnicmp(sourcePath, "$MYDOCS$", 8) == 0) {
		// For platforms with no My Documents folder, just
		// redirect it back to current folder
		Common::strcpy_s(destPath, 256, ".");
		Common::strcat_s(destPath, 256, &sourcePath[8]);
	} else {
		Common::strcpy_s(destPath, 256, sourcePath);
	}

}

void AGSPlatformDriver::ReadPluginsFromDisk(Common::SeekableReadStream *iii) {
	if (getw(iii) != 1)
...
void AGS32BitOSDriver::GetSystemTime(ScriptDateTime *sdt) {
	...
	sdt->hour = td.tm_hour;
	sdt->minute = td.tm_min;
	sdt->second = td.tm_sec;
	sdt->day = td.tm_mday;
	sdt->month = td.tm_mon + 1;
	sdt->year = td.tm_year + 1900;
}
...
byte **IAGSEngine::GetRawBitmapSurface(BITMAP *bmp) {
...
int IAGSEngine::FRead(void *buffer, int32 len, intptr handle) {
	Common::ReadStream *rs = (Common::ReadStream *)handle;
	return rs->read(buffer, len);
}
int IAGSEngine::FWrite(void *buffer, int32 len, intptr handle) {
	Common::WriteStream *ws = (Common::WriteStream *)handle;
...
int IAGSEngine::IsChannelPlaying(int32 channel) {
	return AGS2::IsChannelPlaying(channel);
}
...
void IAGSEngine::QueueGameScriptFunction(const char *name, int32 globalScript, int32 numArgs, int32 arg1, int32 arg2) {
	...
	Common::strcat_s(scNameToRun, name);
...
void pl_read_plugins_from_disk(Common::SeekableReadStream *iii) {
	...
	for (a = 0; a < numPlugins; a++) {
		...
		Common::strcpy_s(apl->filename, buffer);
#ifdef MAC_VERSION
		// replace .dll with .dylib
		char *extPos = strcasestr(apl->filename, ".dll");
		if (extPos)
		{
			Common::strcpy_s(extPos, ".dylib");
		} else
		{
			Common::strcat_s(apl->filename, ".dylib");
		}
		...
		if (apl->dllHandle == NULL)
		{
			...
			char caDylib[512];
			Common::sprintf_s(caDylib, "%s/%s", appDirectory, apl->filename);
			apl->dllHandle = LoadLibrary(caDylib);
			if (apl->dllHandle == NULL)
			{
				// warn and maybe continue anyway
				char caError[512];
				Common::sprintf_s(caError, "Unable to load plugin '%s'\n", buffer);
		...
		apl->dllHandle = LoadLibrary(apl->filename);
		if (apl->dllHandle == NULL) {
			Common::sprintf_s(buffer, "Unable to load plugin '%s'", apl->filename);
			quit(buffer);
		}
#endif
		if (GetProcAddress(apl->dllHandle, "AGS_PluginV2") == NULL) {
			Common::sprintf_s(buffer, "Plugin '%s' is an old incompatible version.", apl->filename);
			quit(buffer);
		}
		...
		if (apl->engineStartup == NULL) {
			Common::sprintf_s(buffer, "Plugin '%s' is not a valid AGS plugin (no engine startup entry point)", apl->filename);
			quit(buffer);
		}
		...
		if (datasize > 0) {
			...
			iii->read(apl->savedata, datasize);
		}
