...
void set_game_speed(int newfps) {
	frames_per_second = newfps;
	time_between_timers = 1000 / newfps;
...
void get_current_dir_path(char *buffer, const char *fileName) {
	...
	{
		Common::sprintf_s(buffer, STD_BUFFER_SIZE, "Compiled\\%s", fileName);
	} else
	{
		Common::strcpy_s(buffer, STD_BUFFER_SIZE, fileName);
	}
}
...
int run_claimable_event(char *tsname, bool includeRoom, int numParams, int param1, int param2, bool *evtWasClaimed) {
	*evtWasClaimed = true;
...
int run_text_script_iparam(ccInstance * sci, char *tsname, int iparam) {
	if ((strcmp(tsname, "on_key_press") == 0) || (strcmp(tsname, "on_mouse_click") == 0)) {
		bool claimed;
		int toret = run_claimable_event(tsname, true, 1, iparam, 0, &claimed);
...
int run_text_script_2iparam(ccInstance * sci, char *tsname, int iparam, int param2) {
	if (strcmp(tsname, "on_event") == 0) {
		bool claimed;
		int toret = run_claimable_event(tsname, true, 2, iparam, param2, &claimed);
...
void Button_SetText(GUIButton * butt, const char *newtx) {
	...
	if (strcmp(butt->text, newtx)) {
		guis_need_update = 1;
		Common::strcpy_s(butt->text, newtx);
	}
}
