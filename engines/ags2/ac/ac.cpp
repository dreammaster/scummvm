...
static const char *heightTestString = "ZHwypgfjqhkilIK";
static const char *tsnames[4] = { NULL, REP_EXEC_NAME, "on_key_press","on_mouse_click" };
static const char *replayTempFile = "~replay.tmp";
...
void show_preload();
void stop_recording();
void save_game_data(Common::WriteStream *, block screenshot);
...
int my_readkey() {
	...
	//Common::sprintf_s(message, "Keypress: %d", gott);
...
int rec_iskeypressed(int keycode) {
	...
	short toret = key[keycode];

	if (play.recording) {
		if (toret != playback_keystate[keycode]) {
			short buff[2] = { (short)keycode, toret };
...
int rec_misbuttondown(int but) {
	...
	short result = misbuttondown(but);
	if (play.recording) {
		if (result != recbutstate[but]) {
			short buff[2] = { (short)but, result };
...
int rec_mgetbutton() {
	...
	if (play.recording) {
		if (result >= 0) {
			short buff[3] = { result, (short)mousex, (short)mousey };
...
void rec_domouse(int what) {
	...
	if (play.recording) {
		...
		if ((mxwas != mousex) || (mywas != mousey)) {
			...
			short buff[2] = { (short)mousex, (short)mousey };
...
void debug_write_console(char *msg, ...) {
	...
	Common::strcpy_s(debug_line[last_debug_line].text, str.c_str());

	ccInstance *curinst = ccGetCurrentInstance();
	if (curinst != NULL) {
		char scriptname[20];
		if (curinst->instanceof == gamescript)
			Common::strcpy_s(scriptname, "G ");
		else if (curinst->instanceof == thisroom.compiled_script)
			Common::strcpy_s(scriptname, "R ");
		else if (curinst->instanceof == dialogScriptsScript)
			Common::strcpy_s(scriptname, "D ");
		else
			Common::strcpy_s(scriptname, "? ");
		Common::sprintf_s(debug_line[last_debug_line].script, "%s%d", scriptname, currentline);
	} else debug_line[last_debug_line].script[0] = 0;

	platform->WriteDebugString("%s (%s)", str.c_str(), debug_line[last_debug_line].script);
...
const char *get_cur_script(int numberOfLinesOfCallStack) {
	...
	if (pexbuf[0] == 0)
		Common::strcpy_s(pexbuf, ccErrorCallStack);
...
bool init_translation(const char *lang) {
	...
	if (lang == NULL) {
		Common::sprintf_s(transFileName, "default.tra");
	} else {
		Common::sprintf_s(transFileName, "%s.tra", lang);
	}

	transFileLoc = ci_find_file(usetup.data_files_dir, transFileName);

	Common::SeekableReadStream *language_file = clibfopen(transFileLoc, "rb");
	free(transFileLoc);
	...
	if (language_file == NULL)
	{
		if (lang != NULL)
		{
			...
			Common::sprintf_s(transFileName, "Compiled\\%s.tra", lang);
	...
	lang_offs_start = language_file->pos();
...
int GetTranslationName(char *buffer) {
	...
	Common::strcpy_s(buffer, STD_BUFFER_SIZE, copyFrom);
...
int Game_ChangeTranslation(const char *newFilename) {
	...
	{
		close_translation();
		Common::strcpy_s(transFileName, "");
		return 1;
	}

	char oldTransFileName[MAX_PATH];
	Common::strcpy_s(oldTransFileName, transFileName);

	if (!init_translation(newFilename))
	{
		Common::strcpy_s(transFileName, oldTransFileName);
		return 0;
	}
...
void DeleteSaveSlot(int slnum) {
	Common::String saveName = g_engine->getSaveStateName(slnum);
...
int Game_SetSaveGameDirectory(const char *newFolder) {
	...
	return 1;
#else
	error("TODO: Game_SetSaveDirectory");
#endif
}
...
void load_new_room(int newnum, CharacterInfo * forchar) {
	...
	Common::sprintf_s(rmfile, "room%d.crm", newnum);
	if (newnum == 0) {
		// support both room0.crm and intro.crm
		Common::SeekableReadStream *inpu = clibfopen(rmfile, "rb");
		if (inpu == NULL)
			Common::strcpy_s(rmfile, "intro.crm");
		else
			delete inpu;
	}
	...
	for (cc = 0; cc < croom->numobj; cc++) {
		...
		if (thisroom.wasversion >= 26)
		{
			Common::strcpy_s(objectScriptObjNames[cc], thisroom.objectscriptnames[cc]);
		} else
		{
			Common::sprintf_s(objectScriptObjNames[cc], "o%s", thisroom.objectscriptnames[cc]);
			ags_strlwr(objectScriptObjNames[cc]);
		...
		if (get_walkable_area_pixel(forchar->x, forchar->y) == 0) {
			if (new_room_pos >= 3000) { // bottom or top of screen
				...
				while (1) {
					...
					int nowhere = 0;
					if (tryleft > thisroom.left) {
						tryleft--; nowhere++;
					}
					if (tryright < thisroom.right) {
						tryright++; nowhere++;
					}
				...
				while (1) {
					...
					int nowhere = 0;
					if (tryleft > thisroom.top) {
						tryleft--; nowhere++;
					}
					if (tryright < thisroom.bottom) {
						tryright++; nowhere++;
					}
					if (nowhere == 0) break;  // no place to go, so leave him
				}
	...
	our_eip = 208;
	if (forchar != NULL) {
		if (thisroom.options[ST_MANDISABLED] == 0) {
			forchar->on = 1;
...
void process_event(EventHappened * evp) {
	if (evp->type == EV_TEXTSCRIPT) {
		...
		if (evp->data2 > -1000) {
			if (inside_script) {
				char nameToExec[50];
				Common::sprintf_s(nameToExec, "!%s", tsnames[evp->data1]);
...
void check_controls() {
	...
	if (kbhit()) {
		...
		if (play.fast_forward) {
			...
			Common::sprintf_s(infobuf, "In room %d %s[Player at %d, %d (view %d, loop %d, frame %d)%s%s%s",
			...
			for (ff = 0; ff < croom->numobj; ff++) {
				...
				Common::sprintf_s(&infobuf[strlen(infobuf)], 900 - strlen(infobuf),
			...
			for (ff = 0; ff < game.numcharacters; ff++) {
				...
				chd = ff;
				Common::sprintf_s(&bigbuffer[strlen(bigbuffer)], STD_BUFFER_SIZE - strlen(bigbuffer),
...
int GetLipSyncFrame(char *curtex, int *stroffs) {
	...
	for (int aa = 0; aa < MAXLIPSYNCFRAMES; aa++) {
		...
		while (tptr[0] != 0) {
			...
			if ((scumm_strnicmp(curtex, tptr, lenthisbit) == 0) && (lenthisbit > bestfit_len)) {
				bestfit = aa;
...
void update_stuff() {
	...
	if (onwalkarea < 0);
		...
		onwalkarea = thisroom.shadinginfo[onwalkarea];
		...
		// count idle time
			...
			if (chi->idleleft == -1) {
				...
				if ((chi->idletime > 0) && (useloop >= maxLoops)) {
					do {
						useloop = g_engine->getRandomNumber() % maxLoops;
...
void replace_macro_tokens(char *statusbarformat, char *cur_stb_text) {
	...
	while (1) {
		...
		if (curptr[0] == '@') {
			...
			Common::strcat_s(cur_stb_text, STD_BUFFER_SIZE, tempo);
		} else {
			tmpm[0] = curptr[0]; tmpm[1] = 0;
			Common::strcat_s(cur_stb_text, STD_BUFFER_SIZE, tmpm);
			curptr++;
		}
	}
}
...
void update_invorder() {
	for (int cc = 0; cc < game.numcharacters; cc++) {
		...
		for (int ff = 0; ff < game.numinvitems; ff++) {
...
int GetRegionAt(int xxx, int yyy) {
	...
	if (hsthere >= MAX_REGIONS) {
		char tempmsg[300];
		Common::sprintf_s(tempmsg, "!An invalid pixel was found on the room region mask (colour %d, location: %d, %d)", hsthere, xxx, yyy);
		quit(tempmsg);
	}
...
void get_script_name(ccInstance * rinst, char *curScrName) {
	if (rinst == NULL)
		Common::strcpy_s(curScrName, STD_BUFFER_SIZE, "Not in a script");
	else if (rinst->instanceof == gamescript)
		Common::strcpy_s(curScrName, STD_BUFFER_SIZE, "Global script");
	else if (rinst->instanceof == thisroom.compiled_script)
		Common::sprintf_s(curScrName, STD_BUFFER_SIZE, "Room %d script", displayed_room);
	else
		Common::strcpy_s(curScrName, STD_BUFFER_SIZE, "Unknown script");
}
...
void draw_fps() {
	...
	abuf = fpsDisplay;
	char tbuffer[60];
	Common::sprintf_s(tbuffer, "FPS: %d", fps);
	wtextcolor(14);
	...
	Common::sprintf_s(tbuffer, "Loop %ld", loopcounter);
...
void start_recording() {
	...
	replay_time = 0;
	Common::strcpy_s(replayfile, "New.agr");
}

void start_replay_record() {
	Common::OutSaveFile *ott = g_system->getSavefileManager()->openForSaving(replayTempFile);
...
void stop_recording() {
	...
	if (replayfile[0] == 0)
		Common::strcpy_s(replayfile, "Untitled");
	if (strchr(replayfile, '.') != NULL)
		strchr(replayfile, '.')[0] = 0;
	Common::strcat_s(replayfile, ".agr");

	Common::OutSaveFile *ooo = g_system->getSavefileManager()->openForSaving(replayfile);
	ooo->write((const byte *)"AGSRecording", 12);
	fputstring(ACI_VERSION_TEXT, ooo);

	int write_version = 2;
	Common::InSaveFile *fsr = g_system->getSavefileManager()->openForLoading(replayTempFile);
	if (fsr != NULL) {
	...
	for (int ff = 0; ff < recsize; ++ff)
		ooo->writeSint16LE(recordbuffer[ff]);

	if (fsr != NULL) {
		putw(1, ooo);  // yes there is a save present
		int lenno = fsr->size();
		char *tbufr = (char *)malloc(lenno);
		fsr->read(tbufr, lenno);
		ooo->write(tbufr, lenno);
		free(tbufr);
		delete fsr;

		g_system->getSavefileManager()->removeSavefile(replayTempFile);
...
void quit(char *quitmsg) {
	...
	if (qmsg[0] == '|'); //qmsg++;
		...
		if (qmsg[0] == '|')
			Common::strcpy_s(alertis, "Abort key pressed.\n\n");
		else if (qmsg[0] == '?') {
			Common::strcpy_s(alertis, "A fatal error has been generated by the script using the AbortGame function. Please contact the game author for support.\n\n");
			qmsg++;
		} else
			Common::strcpy_s(alertis, "An error has occurred. Please contact the game author for support, as this "
				"is likely to be a scripting error and not a bug in AGS.\n"
				"(ACI version " ACI_VERSION_TEXT ")\n\n");

		Common::strcat_s(alertis, get_cur_script(5));

		if (qmsg[0] != '|')
			Common::strcat_s(alertis, "\nError: ");
		else
			qmsg = "";
	} else if (qmsg[0] == '%') {
		qmsg++;

		Common::sprintf_s(alertis, "A warning has been generated. This is not normally fatal, but you have selected "
			"to treat warnings as errors.\n"
			"(ACI version " ACI_VERSION_TEXT ")\n\n%s\n", get_cur_script(5));
	} else Common::strcpy_s(alertis, "An internal error has occurred. Please note down the following information.\n"
...
int String_CompareTo(const char *thisString, const char *otherString, bool caseSensitive) {
	...
	if (caseSensitive) {
		...
		return scumm_stricmp(thisString, otherString);
	}
}
...
int String_StartsWith(const char *thisString, const char *checkForString, bool caseSensitive) {
	...
	if (caseSensitive) {
		...
		return (scumm_strnicmp(thisString, checkForString, strlen(checkForString)) == 0) ? 1 : 0;
	}
}
...
int String_EndsWith(const char *thisString, const char *checkForString, bool caseSensitive) {
	...
	{
		return (scumm_stricmp(&thisString[checkAtOffset], checkForString) == 0) ? 1 : 0;
	}
}
...
const char *String_Replace(const char *thisString, const char *lookForText, const char *replaceWithText, bool caseSensitive) {
	...
	for (int i = 0; i < thisStringLen; i++)
	{
		...
		{
			matchHere = (scumm_strnicmp(&thisString[i], lookForText, strlen(lookForText)) == 0);
		}
...
void adjust_sizes_for_resolution(int filever) {
	...
	for (ee = 0; ee < game.numgui; ee++)
	{
		...
		for (int ff = 0; ff < cgp->numobjs; ff++)
		{
...
int play_speech(int charid, int sndid) {
	...
	char finame[40] = "~";
	Common::strcat_s(finame, get_filename(speech_file));
	Common::strcat_s(finame, "~");
	...
	if (charid >= 0) {
		...
		theScriptName[4] = 0;
		Common::strcat_s(finame, theScriptName);
	} else
	...
	for (ii = 0; ii < numLipLines; ii++) {
		if (scumm_stricmp(splipsync[ii].filename, basefnptr) == 0) {
			curLipLine = ii;
			break;
		}
	}
	...
	while (srcmes[indxsrc] != 0) {
		...
		if ((strncmp(srcp, "@IN", 3) == 0) | (strncmp(srcp, "@GI", 3) == 0)) {
			...
			if (tokentype == 1) {
				...
				Common::sprintf_s(tval, "%d", playerchar->inv[inx]);
			} else {
...
int SaveScreenShot(char *namm) {
	...
	if (strchr(namm, '.') == NULL)
		Common::sprintf_s(fileName, "%s%s.bmp", saveGameDirectory, namm);
	else
		Common::sprintf_s(fileName, "%s%s", saveGameDirectory, namm);
...
void SetObjectView(int obn, int vii) {
	...
	if ((vii < 1) || (vii > game.numviews)) {
		char buffer[150];
		Common::sprintf_s(buffer, "!SetObjectView: invalid view number (You said %d, max is %d)", vii, game.numviews);
		quit(buffer);
	}
	vii--;
...
void pause_sound_if_necessary_and_play_video(const char *name, int skip, int flags) {
	...
	if ((strlen(name) > 3) && (scumm_stricmp(&name[strlen(name) - 3], "ogv") == 0))
	{
...
int FindGUIID(const char *GUIName) {
	for (int ii = 0; ii < game.numgui; ii++) {
		if (strcmp(guis[ii].name, GUIName) == 0)
			return ii;
		if ((guis[ii].name[0] == 'g') && (scumm_stricmp(&guis[ii].name[1], GUIName) == 0))
			return ii;
	}
...
void PlayMP3File(char *filename) {
	...
	if ((channels[useChan] = my_load_static_ogg(pathToFile, 150, doLoop)) != NULL) {
		...
		if (filename != &play.playmp3file_name[0])
			Common::strcpy_s(play.playmp3file_name, filename);
...
int find_word_in_dictionary(char *lookfor) {
	...
	for (j = 0; j < game.dict->num_words; j++) {
		if (scumm_stricmp(lookfor, game.dict->word[j]) == 0) {
...
int FindMatchingMultiWordWord(char *thisword, char **text) {
	...
	do {
		...
		while (is_valid_word_char(tempptr[0])) {
			Common::sprintf_s(chbuffer, "%c", tempptr[0]);
		...
		if (is_valid_word_char(text[0])) {
			...
			if (compareto) {
				...
				if ((word < 0) && (play.bad_parsed_word[0] == 0))
					Common::strcpy_s(play.bad_parsed_word, thisword);
			}
...
void GUIControl_SetWidth(GUIObject * guio, int width) {
	guio->wid = multiply_up_coordinate(width);
	guio->Resized();
	guis[guio->guin].control_positions_changed();
	guis_need_update = 1;
}
...
void GUIControl_SetHeight(GUIObject * guio, int height) {
	guio->hit = multiply_up_coordinate(height);
	guio->Resized();
	guis[guio->guin].control_positions_changed();
	guis_need_update = 1;
}

void GUIControl_SetSize(GUIObject * guio, int width, int height) {
	if ((width < 2) || (height < 2))
		quit("!SetGUIObjectSize: new size is too small (must be at least 2x2)");

	DEBUG_CONSOLE("SetGUIObject %d,%d size %d,%d", guio->guin, guio->objn, width, height);
	GUIControl_SetWidth(guio, width);
	GUIControl_SetHeight(guio, height);
}
...
void SetGUIObjectSize(int ifn, int objn, int width, int height) {
	if ((ifn < 0) || (ifn >= game.numgui))
	...
	GUIControl_SetSize(guis[ifn].objs[objn], width, height);
}
...
void GUI_SetWidth(ScriptGUI * sgui, int width) {
	GUI_SetSize(sgui, width, GUI_GetHeight(sgui));
}

void GUI_SetHeight(ScriptGUI * sgui, int height) {
	GUI_SetSize(sgui, GUI_GetWidth(sgui), height);
}
...
void TextBox_SetText(GUITextBox * texbox, const char *newtex) {
	...
	if (strcmp(texbox->text, newtex)) {
		Common::strcpy_s(texbox->text, newtex);
		guis_need_update = 1;
	}
}
...
int ListBox_FillSaveGameList(GUIListBox * listbox) {
	...
	char searchPath[260];
	Common::sprintf_s(searchPath, "%s""agssave.*", saveGameDirectory);
...
void InvWindow_SetItemHeight(GUIInv * guii, int height) {
	guii->itemHeight = height;
	guii->Resized();
}
...
int __Rand(int upto) {
	...
	return g_engine->getRandomNumber() % upto;
}
...
/*void GetLanguageString(int indxx,char*buffr) {
  ...
  if (bptr==NULL) Common::strcpy_s(buffr,"[language string error]");
...
void script_debug(int cmdd, int dataa) {
	...
	if (cmdd == 0) {
		...
		Common::sprintf_s(toDisplay, "Adventure Game Studio run-time engine[ACI version " ACI_VERSION_TEXT
		...
		if (play.seperate_music_lib)
			Common::strcat_s(toDisplay, "[AUDIO.VOX enabled");
		if (play.want_speech >= 1)
			Common::strcat_s(toDisplay, "[SPEECH.VOX enabled");
		if (transtree != NULL) {
			Common::strcat_s(toDisplay, "[Using translation ");
			Common::strcat_s(toDisplay, transFileName);
		}
		if (opts.mod_player == 0)
			Common::strcat_s(toDisplay, "[(mod/xm player discarded)");
		Display(toDisplay);
	...
	} else if (cmdd == 3)
	{
		...
		{
			char inroomtex[80];
			Common::sprintf_s(inroomtex, "!Enter new room: (in room %d)", displayed_room);
			setup_for_dialog();
...
int _sc_scumm_stricmp (char *s1, char *s2) {
  return scumm_stricmp (get_translation (s1), get_translation(s2));
}*/
...
void my_sprintf(char *buffer, const char *fmt, va_list ap) {
	...
	while (1) {
		...
		else if (endptr[-1] == 's')
		{
			...
			Common::sprintf_s(spfbuffer, fmtstring, theArg);
...
InteractionVariable *FindGraphicalVariable(const char *varName) {
	...
	for (ii = 0; ii < numGlobalVars; ii++) {
		if (scumm_stricmp(globalvars[ii].name, varName) == 0)
			return &globalvars[ii];
	}
	for (ii = 0; ii < thisroom.numLocalVars; ii++) {
		if (scumm_stricmp(thisroom.localvars[ii].name, varName) == 0)
...
int GetGraphicalVariable(const char *varName) {
	...
	if (theVar == NULL) {
		...
		Common::sprintf_s(quitmessage, "!GetGraphicalVariable: interaction variable '%s' not found", varName);
		quit(quitmessage);
		return 0;
	}
	return theVar->value;
...
void SetGraphicalVariable(const char *varName, int p_value) {
	...
	if (theVar == NULL) {
		char quitmessage[120];
		Common::sprintf_s(quitmessage, "!SetGraphicalVariable: interaction variable '%s' not found", varName);
		quit(quitmessage);
	} else
		...
		case 1:  // Run script
		{
			...
			UPDATE_MP3
				if ((strstr(evblockbasename, "character") != 0) || (strstr(evblockbasename, "inventory") != 0)) {
					...
					if (inside_script) {
						char funcName[60];
						Common::strcpy_s(funcName, "|");
						Common::strcat_s(funcName, make_ts_func_name(evblockbasename, evblocknum, nicl->command[i].data[0].val));
...
void run_unhandled_event(int evnt) {
	...
	int evtype = 0;
	if (scumm_strnicmp(evblockbasename, "hotspot", 7) == 0) evtype = 1;
	else if (scumm_strnicmp(evblockbasename, "object", 6) == 0) evtype = 2;
	else if (scumm_strnicmp(evblockbasename, "character", 9) == 0) evtype = 3;
	else if (scumm_strnicmp(evblockbasename, "inventory", 9) == 0) evtype = 5;
	else if (scumm_strnicmp(evblockbasename, "region", 6) == 0)
...
int run_interaction_script(InteractionScripts * nint, int evnt, int chkAny, int isInv) {
	...
	UPDATE_MP3
		if ((strstr(evblockbasename, "character") != 0) || (strstr(evblockbasename, "inventory") != 0)) {
			...
			if (inside_script) {
				char funcName[60];
				Common::sprintf_s(funcName, "|%s", nint->scriptFuncNames[evnt]);
				curscript->run_another(funcName, 0, 0);
			} else
...
void setup_script_exports() {
	...
	scAdd_External_Symbol("Common::strcat_s", (void *)_sc_strcat);
...
void setup_exports(char *expfrom) {
	...
	while (expfrom[0] != 0) {
		...
		int iid = 0;
		while (expfrom[iid] != '(') {
			namof[iid] = expfrom[iid]; iid++;
		}
		namof[iid] = 0;
		Common::strcat_s(temphdr, "export ");
		Common::strcat_s(temphdr, namof);
		Common::strcat_s(temphdr, ";\r\n");
	}
	int aa;
	for (aa = 0; aa < game.numcharacters - 1; aa++) {
		if (game.chars[aa].scrname[0] == 0) continue;
		Common::strcat_s(temphdr, "#define ");
...
void compile_room_script() {
	...
	if ((ccError != 0) || (roominst == NULL)) {
		char thiserror[400];
		Common::sprintf_s(thiserror, "Unable to create local script: %s", ccErrorString);
		quit(thiserror);
	}
...
void init_game_settings() {
	...
	Common::strcpy_s(play.game_name, game.gamename);
	...
	while (!fin.eos()) {
		...
		if (scumm_strnicmp(wantsect, templine, strlen(wantsect)) == 0) {
			while (!feof(fin)) {
				...
				if (scumm_strnicmp(templine, entry, strlen(entry)) == 0) {
...
void read_config_file(char *argv0) {
	...
	if (ppp == NULL) {
		...
		Common::strcpy_s(conffilebuf, argv0);
		...
		Common::strcat_s(ac_config_file, "\\acsetup.cfg");
	...
	if (ppp != NULL) {
		Common::strcpy_s(filetouse, ac_config_file);
		fclose(ppp);
		...
		if (repfile != NULL) {
			Common::strcpy_s(replayfile, repfile);
			free(repfile);
			play.playback = 1;
		} else
			play.playback = 0;
...
int ags_main2(int argc, char *argv[]) {
	our_eip = -999;
	cfopenpriority = 2;
	int ee;
	play.recording = 0;
...
int initialize_engine(int argc, char *argv[]) {
	...
	if (argc > 1) {
		if (scumm_stricmp(argv[1], "--setup") == 0) {
	...
	if ((errcod != 0) && (change_to_game_dir == 0)) {
		...
		if (errcod) {
			//Common::sprintf_s(gamefilenamebuf,"%s\\ac2game.ags",usetup.data_files_dir);
			free(game_file_name);
		...
		if (((strchr(game_file_name, '/') != NULL) ||
			...
			(scumm_stricmp(usetup.data_files_dir, ".") == 0)) {
	...
	if (game.saveGameFolderName[0] != 0)
	{
		...
		Common::sprintf_s(newDirBuffer, "$MYDOCS$/%s", game.saveGameFolderName);
	...
	if (switch_to_graphics_mode(initasx, initasy, scrnwid, scrnhit, firstDepth, secondDepth))
	{
		...
		if (((usetup.gfxFilterID == NULL) ||
			(scumm_stricmp(usetup.gfxFilterID, "None") == 0)) &&
			(scrnwid == 320))
		{
	...
	scsystem.viewport_height = divide_down_coordinate(scrnhit);
	Common::strcpy_s(scsystem.aci_version, ACI_VERSION_TEXT);
