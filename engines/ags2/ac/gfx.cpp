...
public:
	MouseGetPosCallbackImpl(ScalingGFXFilter *gfxFilter) {
		_callbackFilter = gfxFilter;
	}

	void AdjustPosition(int *x, int *y) override {
		_callbackFilter->AdjustPosition(x, y);
	}
};

extern int Init_2xSaI(int d);
extern void Super2xSaI(BITMAP * src, BITMAP * dest, int s_x, int s_y, int d_x, int d_y, int w, int h);
...
ScalingGFXFilter::ScalingGFXFilter(int multiplier, bool justCheckingForSetup) : GFXFilter() {
	...
	Common::sprintf_s(filterName, "%d" "x nearest-neighbour filter[", multiplier);
	Common::sprintf_s(filterID, "StdScale%d", multiplier);
}
...
D3DGFXFilter::D3DGFXFilter(bool justCheckingForSetup) : ScalingGFXFilter(1, justCheckingForSetup) {
	Common::sprintf_s(filterName, "");
	Common::sprintf_s(filterID, "None");
}
...
struct AAD3DGFXFilter : D3DGFXFilter {
	...
	AAD3DGFXFilter(int multiplier, bool justCheckingForSetup) : D3DGFXFilter(multiplier, justCheckingForSetup) {
		Common::sprintf_s(filterName, "%d" "x anti-aliasing filter[", multiplier);
		Common::sprintf_s(filterID, "AAx%d", multiplier);
	}
...
BITMAP *AllegroGFXFilter::ScreenInitialized(BITMAP *scr, int fakeWidth, int fakeHeight) {
	realScreen = scr;
	return scr;
}
...
public:
	...
	virtual BITMAP *ScreenInitialized(BITMAP *scr, int fakeWidth, int fakeHeight) {
		realScreen = scr;
		realScreenSizedBuffer = create_bitmap_ex(bitmap_color_depth(scr), scr->w, scr->h);
		fakeScreen = create_bitmap_ex(bitmap_color_depth(scr), fakeWidth, fakeHeight);
		return fakeScreen;
	}
...
public:
	...
	virtual BITMAP *ScreenInitialized(BITMAP *scr, int fakeWidth, int fakeHeight) {
		realScreen = scr;
		realScreenBuffer = create_bitmap(scr->w, scr->h);
		realScreenSizedBuffer = create_bitmap_ex(bitmap_color_depth(scr), scr->w, scr->h);
		fakeScreen = create_bitmap_ex(bitmap_color_depth(scr), fakeWidth, fakeHeight);
		InitLUTs();
		return fakeScreen;
