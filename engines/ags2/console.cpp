...
#include "ags2/shared/gfx/allegro_bitmap.h"
...
AGSConsole::AGSConsole(AGSEngine *vm) : GUI::Debugger(), _vm(vm), _logOutputTarget(nullptr), _agsDebuggerOutput(nullptr) {
	...
	_agsDebuggerOutput = _GP(DbgMgr).RegisterOutput("ScummVMLog", _logOutputTarget, AGS2::AGS::Shared::kDbgMsg_None);
}
...
struct LevelName {
	const char *name;
	AGS2::AGS::Shared::MessageType level;
};

static const LevelName levelNames[] = {
	{"none", AGS2::AGS::Shared::kDbgMsg_None},
	{"alerts", AGS2::AGS::Shared::kDbgMsg_Alert},
	{"fatal", AGS2::AGS::Shared::kDbgMsg_Fatal},
	{"errors", AGS2::AGS::Shared::kDbgMsg_Error},
	{"warnings", AGS2::AGS::Shared::kDbgMsg_None},
	{"info", AGS2::AGS::Shared::kDbgMsg_Info},
	{"debug", AGS2::AGS::Shared::kDbgMsg_Debug},
	{nullptr, AGS2::AGS::Shared::kDbgMsg_None}
};
...
static const GroupName groupNames[] = {
	{"Main", AGS2::AGS::Shared::kDbgGroup_Main},
	{"Game", AGS2::AGS::Shared::kDbgGroup_Game},
	{"Script", AGS2::AGS::Shared::kDbgGroup_Script},
	{"SpriteCache", AGS2::AGS::Shared::kDbgGroup_SprCache},
	{"ManObj", AGS2::AGS::Shared::kDbgGroup_ManObj},
	{nullptr, (AGS2::uint32_t)-1}
};
...
bool AGSConsole::Cmd_setDebugGroupLevel(int argc, const char **argv) {
	...
	bool found = false;
	AGS2::uint32_t group = parseGroup(argv[1], found);
	if (!found) {
	...
	AGS2::AGS::Shared::MessageType level = parseLevel(argv[2], found);
	if (!found) {
...
const char *AGSConsole::getVerbosityLevel(AGS2::uint32_t groupID) const {
	int i = 1;
...
AGS2::uint32_t AGSConsole::parseGroup(const char *name, bool &found) const {
	int i = 0;
	...
	found = false;
	return (AGS2::uint32_t)-1;
}

AGS2::AGS::Shared::MessageType AGSConsole::parseLevel(const char *name, bool &found) const {
	int i = 0;
	while (levelNames[i].name != nullptr) {
	...
	found = false;
	return AGS2::AGS::Shared::kDbgMsg_None;
}
...
bool AGSConsole::Cmd_SetScriptDump(int argc, const char **argv) {
	...
	if (strcmp(argv[1], "on") == 0 || strcmp(argv[1], "true") == 0)
		...
		AGS2::ccSetOption(SCOPT_DEBUGRUN, 0);
	return true;
}
...
bool AGSConsole::Cmd_getSpriteInfo(int argc, const char **argv) {
	...
	AGS2::Shared::Bitmap *sprite = _GP(spriteset)[spriteId];
	if (!sprite) {
...
void LogOutputTarget::PrintMessage(const AGS2::AGS::Shared::DebugMessage &msg) {
	LogMessageType::Type msgType = LogMessageType::kInfo;
	switch (msg.MT) {
	case AGS2::AGS::Shared::kDbgMsg_None:
		return;
	case AGS2::AGS::Shared::kDbgMsg_Alert:
	case AGS2::AGS::Shared::kDbgMsg_Fatal:
	case AGS2::AGS::Shared::kDbgMsg_Error:
		msgType = LogMessageType::kError;
		break;
	case AGS2::AGS::Shared::kDbgMsg_Warn:
		msgType = LogMessageType::kWarning;
		break;
	case AGS2::AGS::Shared::kDbgMsg_Info:
		msgType = LogMessageType::kInfo;
		break;
	case AGS2::AGS::Shared::kDbgMsg_Debug:
