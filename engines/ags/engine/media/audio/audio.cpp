...
int _G(reserved_channel_count) = 0;
...
int _G(current_music_type) = 0;
// _G(crossFading) is >0 (channel number of new track), or -1 (old
// track fading out, no new track)
int _G(crossFading) = 0, _G(crossFadeVolumePerStep) = 0, _G(crossFadeStep) = 0;
int _G(crossFadeVolumeAtStart) = 0;
SOUNDCLIP *_G(cachedQueuedMusic) = nullptr;

//=============================================================================
// Music update is scheduled when the voice speech stops;
// we do a small delay before reverting any volume adjustments
static bool _G(music_update_scheduled) = false;
static auto _G(music_update_at) = AGS_Clock::now();
...
void postpone_scheduled_music_update_by(std::chrono::milliseconds duration) {
    if (!_G(music_update_scheduled)) { return; }
    _G(music_update_at) += duration;
}

void process_scheduled_music_update() {
    if (!_G(music_update_scheduled)) { return; }
    if (_G(music_update_at) > AGS_Clock::now()) { return; }
...
int calculate_max_volume() {
    ...
    int newvol=_GP(play).music_master_volume + ((int)_GP(_GP(thisroom)).Options.MusicVolume) * LegacyRoomVolumeFactor;
