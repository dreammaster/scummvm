...
void OpenALDecoder::PollBuffers() {
	...
	while (!EOS_) {
		...
		if (buffersQueued >= SourceMinimumQueuedBuffers) {
			break;
		}
...
OpenALDecoder::OpenALDecoder(ALuint source, std::future<std::vector<char>> sampleBufFuture, AGS::Shared::String sampleExt, bool repeat)
	: source_(source), sampleBufFuture_(std::move(sampleBufFuture)), sampleExt_(sampleExt), repeat_(repeat) {

}

void OpenALDecoder::Poll() {
	if (playState_ == AudioCorePlayState::PlayStateError) {
		return;
	}

	if (playState_ == AudioCorePlayState::PlayStateInitial) {

		if (sampleBufFuture_.wait_for(std::chrono::seconds(0)) != std::future_status::ready) {
			return;
		}
		...
		if (!sample) {
			playState_ = AudioCorePlayState::PlayStateError; return;
		}
	...
	if (playState_ != PlayStatePlaying) {
		return;
	}
