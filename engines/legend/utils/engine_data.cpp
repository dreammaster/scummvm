...
bool DataArchive::hasFile(const Common::Path &path) const {
	...
	return _zip->hasFile(pair._name1.c_str()) || _zip->hasFile(pair._name2.c_str());
}
...
Common::SeekableReadStream *DataArchive::createReadStreamForMember(const Common::Path &path) const {
	...
	if (hasFile(path)) {
		Pair pair = innerToPublic(name);
		rs = _zip->createReadStreamForMember(pair._name1.c_str());
		if (!rs)
			rs = _zip->createReadStreamForMember(pair._name2.c_str());
	}
...
bool load_engine_data(const Common::String &subfolder, bool isEarlyGame,
	...
	if (ConfMan.hasKey("extrapath")) {
		if ((rootFolder = Common::FSNode(ConfMan.get("extrapath").c_str())).exists()
	...
	if (!f.isOpen()) {
		if (!Common::File::exists(DATA_FILENAME) ||
			...
			!f.open(Common::String::format("%s/version.txt", subfolder.c_str()).c_str(), *dataArchive)) {
			delete dataArchive;
