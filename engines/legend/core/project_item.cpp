...
ProjectItem *ProjectItem::loadData(SimpleFile *file) {
	...
	do {
		Common::String entryString = file->readString();
		...
		if (entryString == "ALONG") {
			...
			item = dynamic_cast<TreeItem *>(SaveableObject::createInstance(entryString));
			assert(item);
			...
			if (root) {
				...
				root = dynamic_cast<ProjectItem *>(item);
				assert(root);
			}
...
void ProjectItem::saveData(SimpleFile *file, TreeItem *item) const {
	while (item) {
