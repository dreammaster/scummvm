...
void PicFile::readPalette(uint bitDepth) {
	Screen &screen = *g_engine->_screen;
	byte pal[Graphics::PALETTE_SIZE];
	const int PALETTE_SIZES[5] = { 0, 16, 48, Graphics::PALETTE_SIZE, 48 };
	int palSIze = g_engine->isLater() ? Graphics::PALETTE_SIZE : PALETTE_SIZES[bitDepth];

	// Read in the palette
	Common::fill(pal, pal + Graphics::PALETTE_SIZE, 0);
...
void PicFile::loadIndex(int fileNumber) {
	...
	for (idx = 0; idx < index._entries.size(); ++idx) {
		...
		uint palSize = (e._flags & PIC_HAS_PALETTE) ? Graphics::PALETTE_SIZE : 0;
...
int Picture::decode(Common::SeekableReadStream *inStream) {
	...
	if (g_engine->isLater()) {
		...
		if (!result) {
			...
			assert((outStream->size() * 2) == (uint)(((s.w + 1) / 2) * 2 * s.h));
...
bool PictureArray::load(uint picNum, uint frameNum) {
	Picture *pic = g_engine->_picFile->load(picNum, frameNum);
	if (pic)
		push_back(pic);
	return pic != nullptr;
}
...
size_t PictureArray::loadRange(size_t count, uint picNum, uint frameNum) {
	return g_engine->_picFile->loadRange(*this, count, picNum, frameNum);
}
