...
#include "ultima/ultima0/game/dungeon.h"
#include "ultima/ultima0/game/town.h"
#include "ultima/ultima0/game/world.h"
#include "ultima/ultima0/game/globals.h"
#include "ultima/ultima0/game/monst.h"
#include "ultima/ultima0/game/british.h"
		...
		if (Moved != 0)
		{
			...
			if (n == DT_GOLD)				/* Gold here */
			{
				d->Map[p->Dungeon.x]		/* Remove the gold */
							[p->Dungeon.y] = 0;
				DRAWText("Gold !!!!!\n");
				z = (RANDOM() % (5*p->Level))	/* Calculate amount */
									+ p->Level;
				DRAWText("%d pieces of eight ",z);
				p->Attr[AT_GOLD] += z;		/* Add to total */
				if (z > 0)					/* Object ? */
				{
					z = RANDOM()%(p->Objects);/* Decide which object */
