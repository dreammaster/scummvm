    ...
    if (map.music[*pmap] != 0)
    {
        ...
        if (map.music[*pmap] > 1000) 
        {
            ...
            if (cd_inserted)
            {
                ...
                PlayCD(g_hWnd, map.music[*pmap]-1000);
            } else
            {
...
if (dwReturn = mciSendCommand(wDeviceID, MCI_SET, 
    ...
    for(i=1; i<=iNumTracks; i++)    
    {
        ...
        pMem[i-1] = mciStatusParms.dwReturn;        wsprintf(szTempString, 
            "Track %2d - %02d:%02d:%02d\n", i,
            MCI_MSF_MINUTE(pMem[i-1]),             MCI_MSF_SECOND(pMem[i-1]), 
    ...
    if ( (MCI_MSF_MINUTE(pMem[12-1]) == 1)  && (MCI_MSF_SECOND(pMem[12-1]) == 10)  )
    {
        //registered!
        regcode = mycode;
        sound_support = mcc;
    ...
    if ( (MCI_MSF_MINUTE(pMem[12-1]) == 1)  && (MCI_MSF_SECOND(pMem[12-1]) == 12)  )
    {
        //registered!
        regcode = mycode;
        sound_support = mcc;
...
bool playing( int sound)
{
    ...
    pdsb->GetStatus(&crap);
    if (crap == DSBSTATUS_PLAYING) return(true); else return(false);
...
madeit:    
    ...
    */
    ...
    if (plus == 0)
        soundbank[i]->SetFrequency(min); else
        ...
        if (callback[k].active)
        {
            ...
            } else
            {
                ...
                if (callback[k].timer == 0)
                {
                    ...
                    if (callback[k].max > 0)    
                        callback[k].timer = thist + (rand() % callback[k].max)+callback[k].min;
                    else callback[k].timer = thist + callback[k].min;
