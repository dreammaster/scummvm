...
HRES Resources::loadResource(ULONG resource, ULONG attrib) {
	HD_entry *sel;
	HRES entry;
...
void Resources::loadString(byte *array, uint string) {
	...
	array_offset = array - (byte *)addr(objects[Interpreter::_currentThis]);

	handle = get_resource_handle(string, DA_DEFAULT);

	lock(handle);

	new_array = (byte *)add_ptr(addr(objects[Interpreter::_currentThis]), array_offset);

	ptr = (byte *)addr(handle);

	switch (READ_LE_UINT16(ptr)) {
	case ':S':
		far_memmove(new_array, ptr + 2, size(handle) - 2L);
		break;
...
HRES Resources::get_resource_handle(ULONG resource, ULONG attrib) {
	...
	if (dir == NULL) {
		...
		dir->OE = resource;
		dir->handle = loadResource(resource, attrib);
		dir->thunk = HRES_NULL;
	}
...
HRES Resources::create_instance(ULONG object) {
	...
	if ((entry == NULL) || (entry->thunk == HRES_NULL)) {
		thunk = construct_thunk(this, object);
...
HRES Resources::construct_thunk(Resources *lnk, ULONG object) {
	...
	while (class1 != (ULONG)-1) {
		...
		mcnt = 0;
		dict = Interpreter::first(addr(expt[depth]));
		while ((dict = Interpreter::iterate(addr(expt[depth]), dict, &tag, &def)) != 0) {
			switch (tag[0]) {
			case 'M':               // Message
				++thdr._maxMsg;
				++mcnt;
		...
		dict = Interpreter::first(addr(impt[depth]));
		while ((dict = Interpreter::iterate(addr(impt[depth]), dict, (const char **)&tag, &def)) != 0) {
			switch (tag[0]) {
			case 'C':               // Code
	...
	*(THDR *)addr(thunk) = thdr;
	...
	while (i >= 0) {
		...
		dict = Interpreter::first(addr(impt[i]));
		while ((dict = Interpreter::iterate(addr(impt[i]), dict, &tag, &def)) != 0) {
			tagbase = (const char *)addr(impt[i]);
			switch (tag[0]) {
			case 'C':               // Code
				offset = ascnum(Interpreter::lookup(HCRFD, &tag[2]));
				if (offset == (UWORD)-1)
			...
			case 'L':               // Long
				...
				while (xclass != (ULONG)-1) {
					...
					if (!found) {
						...
						offset = ascnum(Interpreter::lookup(xexpt, tag));
